...
Java_com_orca_app_shared_JiebaEngine_nativeInit(
...
Java_com_orca_app_shared_JiebaEngine_nativeSegment(
...
JNIEXPORT jobjectArray JNICALL
Java_com_orca_app_shared_JiebaEngine_nativeSegmentForSearch(
    JNIEnv* env, jobject /* thiz */, jstring text) {
    std::lock_guard<std::mutex> lock(g_mutex);
    if (!g_state) {
...
// JiebaState.
JNIEXPORT jobjectArray JNICALL
Java_com_orca_app_shared_JiebaEngine_nativePosTag(
    JNIEnv* env, jobject /* thiz */, jstring text) {
    std::lock_guard<std::mutex> lock(g_mutex);
    if (!g_state) {
