...
class AnthropicProvider(
    ...
    internal fun resolveSystemPrompt(userPrompt: String?): JSONArray? {
        val claudeCodePrefix = com.i.app.auth.ClaudeOAuthManager.ANTHROPIC_OAUTH_IDENTIFIER_PROMPT
        if (isOAuth) {
    ...
    private fun buildRequest(bodyStr: String, body: JSONObject): Request {
        ...
        // the branded I UA on the regular apiKey path, but on the OAuth
        ...
        builder.applyUserAgentOverride(
            customUserAgent,
            defaultUserAgent = if (isOAuth) null else com.i.app.provider.IUserAgent.DEFAULT,
        )
