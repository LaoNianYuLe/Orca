package com.i.app.sandbox.offload
...
import com.i.app.provider.safeOptString
import com.i.app.sandbox.NativeOffloadHandler
import com.i.app.sandbox.NativeOffloadRequest
import com.i.app.sandbox.NativeOffloadResult
...
/**
 * `i-model-use` — list, search, and invoke LLM models from Alpine shell.
 * Mirrors iOS ModelUseOffload.m + ModelUseOffloadBridge.swift.
 *
 * Subcommands:
 *   i-model-use list [--provider <name>] [--modality <m>]
 *   i-model-use search <query> [--provider <name>] [--modality <m>]
 *   i-model-use run --model <id_or_name> [--input <path>] [--output <path>]
...
class ModelUseOffloadHandler(
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional[0]) {
                ...
                else -> NativeOffloadResult(2, "i-model-use: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: Throwable) {
    ...
    private fun cmdSearch(args: OffloadArgs): NativeOffloadResult {
        val query = args.positional.getOrNull(1)
            ?: return NativeOffloadResult(
                2,
                "i-model-use search: no query. Usage: i-model-use search <query>\n",
            )
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        val modelArg = args.get("model")
            ?: return NativeOffloadResult(2, "--model is required. Usage: i-model-use run --model <id_or_name>\n")
            ...
            ?: return NativeOffloadResult(
                ...
                JSONObject().put("error", "model_not_found")
                    .put(
                        "message",
                        if (providerFilter != null)
                            "No model '$modelArg' under provider '$providerFilter'. Use 'i-model-use list' to see available combinations."
                        else
                ...
                JSONObject().put("error", "invalid_output_path")
                    .put(
                        ...
                        "--output must be an absolute path (e.g. /var/i/workspace/out.jpg). " +
                            ...
                            "i-model-use does not inherit the shell's working directory.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        if (requiredModality != null) {
            ...
            return NativeOffloadResult(
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put("message", "Model '${entry.model.displayName}' does not support $requiredModality. Supported modalities: ${supported.joinToString(", ")}. Use 'i-model-use list' to find a model with the required capability.")
                    .toString() + "\n",
            )
        }
        ...
        val inputText = when {
            args.get("input") != null -> readLinuxPath(args.get("input")!!)
                ?: return NativeOffloadResult(
                    2,
                    "i-model-use run: cannot read --input '${args.get("input")}'\n",
                )
            else -> ""
        }
        val parsed = try {
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put(
                        ...
                        "Model '${entry.model.displayName}' does not support audio_input, " +
                            ...
                            "Use 'i-model-use list --modality audio' to find an audio-capable model.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        val instance = providerRepository.instance(entry.providerInstanceId)
            ?: return NativeOffloadResult(2, "i-model-use run: provider instance not found\n")
        ...
        if (hasAudioInput && provider !is com.i.app.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.i.app.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.i.app.provider.openai.OpenAIProvider)?.let { openAI ->
        ...
        if (outputPath != null) {
            ...
            val hostFile = sessionScopedHostFile(outputPath, sessionId)
                ...
                ?: return NativeOffloadResult(
                    2,
                    "i-model-use run: cannot resolve --output '$outputPath'\n",
                )
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.i.app.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.i.app.data.model.ImageEndpointMode.auto
            "images_generations", "images-generations", "images-gen" ->
                com.i.app.data.model.ImageEndpointMode.imagesGenerations
            "chat_completions", "chat-completions", "chat" ->
                com.i.app.data.model.ImageEndpointMode.chatCompletions
            else -> null
        }
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.i.app.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        entry: ModelEntry,
        instance: com.i.app.data.model.ProviderInstance,
        provider: com.i.app.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.i.app.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.i.app.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.i.app.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.i.app.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.i.app.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.i.app.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.i.app.data.model.LLMError.ProviderError ||
                e is com.i.app.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.i.app.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.i.app.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.i.app.offload.ModelUseManager.entryDict(
            entry,
    ...
    private fun resolveImageUrl(url: String): LLMMessage.ImagePart {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            throw ImageInputError(
                "http(s):// image URLs are not supported by i-model-use. " +
    ...
    companion object {
        ...
        private const val USAGE_HINT =
            "To invoke a model, pass `--model <model_id>` to `i-model-use run`. " +
        ...
        private const val HELP = """i-model-use — list, search, and invoke LLM models

Usage:
  i-model-use list [--provider <name>] [--modality <mod>]
  i-model-use search <query> [--provider <name>] [--modality <mod>]
  i-model-use run --model <id_or_name> [--provider <label_or_id>]
...
Examples:
  i-model-use list
  i-model-use list --modality image_input
  ...
  i-model-use run --model gemini-2.5-flash --system 'You are a poet' \
