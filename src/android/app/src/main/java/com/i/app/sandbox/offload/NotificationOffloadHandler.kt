...
import com.i.app.offload.INotificationListenerService
import com.i.app.offload.OffloadPermissionManager
import com.i.app.offload.ScheduledNotificationReceiver
import com.i.app.offload.ScheduledNotificationStore
...
class NotificationOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun handleSend(args: OffloadArgs): NativeOffloadResult {
                ...
                if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                    ...
                    r = OffloadPermissionManager.requestSettingsGate(
                        OffloadPermissionManager.SettingsGateRequest(
                            ...
                            message = "Orca needs notification permission to send notifications. Open Settings to allow it.",
        ...
        val oemHint = if (OsCompat.isHuawei || OsCompat.isXiaomi) {
            "On ${OsCompat.oemLabel()}, banner notifications may be disabled by default — the user can enable them in Settings → Notifications for Orca."
        } else null
        val data = JSONObject()
            .put("id", id)
            .put("title", title)
...
Notes:
  ...
  - list needs Notification access (Settings → Apps → Special app
    access → Notification access → I). First list call opens it.
"""
    }
}
