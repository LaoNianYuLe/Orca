package com.i.app.tools

import com.i.app.browser.BrowserAction
import com.i.app.data.model.AgentToolDefinition
import com.i.app.data.model.AgentToolParam
...
object AgentTools {
    ...
    private fun browserUseDefinition(): AgentToolDefinition = AgentToolDefinition(
        ...
        description = "Control a web browser with up to 3 tabs. " +
            "Do NOT use this tool for i:// action URLs (open_terminal, views, settings) — those are app deep links, use Markdown links in chat instead. " +
            "The browser supports both web URLs and i:// resource URLs. Use i:// URLs to preview session files (e.g. navigate to i://workspace/index.html). " +
            "Sub-resources (JS, CSS, images, fonts) referenced via i:// absolute paths or relative paths within HTML pages resolve correctly. " +
            ...
            "fetch to download files/resources using the page's session (returns metadata and a i:// URL), " +
            ...
            "Use get_cookies to retrieve cookies for the current page URL / current site root domain only (including HttpOnly cookies). get_cookies supports optional 'keywords' (filter by cookie name) and 'fuzzy' (true=contains match, false=exact match, default true). It returns only a summary and an offload env file path — raw cookie values are NOT included in the tool response. To reuse cookies in shell commands: `. /var/i/offloads/env_cookies_xxx.sh && command`. You may define alias variables when needed. " +
