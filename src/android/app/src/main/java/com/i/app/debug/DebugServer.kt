...
class DebugServer(
        ...
        socket.use { s ->
            try {
                ...
                if (!isAuthorized(isLoopback, providedToken, authToken)) {
                    ...
                    sendResponse(writer, 401, rpcHandler.errorJSON(-32000, "Unauthorized — send X-I-Token (see `adb shell run-as com.i.app cat files/debug_server_token`)"))
                    return
                }
                ...
                if (method == "GET") {
                    ...
                    when (path) {
                        ...
                        "/skill/examples/python", "/skill/examples/i_rpc_android.py" -> {
                            sendSkillAsset(writer, "examples/i_rpc_android.py", "text/x-python; charset=utf-8")
                            return
                        }
    ...
    private fun sendSkill(writer: PrintWriter, wantsHuman: Boolean) {
        ...
        val py = readSkillAsset("examples/i_rpc_android.py") ?: ""
