 ...
 * shows the I launcher logo, the running tool's label, and a short
 * status — same data the FGS notification reads
 * (`SessionActivityTracker.currentToolName` / `currentToolStatus`).
 *
 * T-bg-overlay-polish: the per-tool icon was replaced with the I
 ...
 *
 *   - I foreground OR no tool running OR user toggle OFF OR no perm → hidden
 *   - otherwise → shown, content updated on each tool-status change
 *
 * Position persists in [com.i.app.data.repository.BackgroundSettingsRepository]
 * across process restarts; first run defaults to the bottom-left corner
 * (10 dp from each edge). Tap dismisses to bring I back to the
...
class ToolOverlayController(private val context: Context) {
    ...
    private fun updateContent(
        ...
        // through verbatim would render a noisy "I / Idle" capsule that
    ...
    private fun onTap() {
        try {
            ...
            val launchIntent = Intent(
                context,
                Class.forName("com.i.app.MainActivity"),
            ).apply {
