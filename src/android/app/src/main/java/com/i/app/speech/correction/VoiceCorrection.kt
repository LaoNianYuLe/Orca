...
import com.i.app.shared.TextSegmenter
...
object VoiceCorrection {
    ...
    fun ensureInitialized(context: Context) {
        ...
        // IApp — AppDatabase.getInstance is already the shared handle.
        val dao = runCatching {
            com.i.app.data.db.AppDatabase.getInstance(app).chatDao()
        }.getOrNull()
        if (dao != null) {
    ...
    fun buildConversationContext(
        context: Context,
        messages: List<com.i.app.ui.chat.ChatMessage>,
    ): ConversationContext {
        ensureInitialized(context)
