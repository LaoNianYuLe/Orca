...
internal object ProviderDebugMethods {
            ...
            if (includeMembers) {
                ...
                for (memberId in group.memberEntryIds) {
                    ...
                    val entry = cfg.modelEntries.find { it.id == memberId }
                        ?: com.i.app.data.model.SystemVoiceEntries.resolve(memberId)
                        ?: continue
    ...
    suspend fun quickTest(context: Context, params: JSONObject): JSONObject {
        ...
        val entry = repo.config.value.modelEntries.find { it.id == entryId }
            ?: com.i.app.data.model.SystemVoiceEntries.resolve(entryId)
            ?: throw RPCException(-32602, "Model entry not found: $entryId")

        val requested = params.optJSONArray("kinds")
        val kinds: List<com.i.app.ui.components.QuickTestKind> = if (requested == null) {
            com.i.app.ui.components.applicableKinds(entry)
        } else {
            (0 until requested.length()).mapNotNull { i ->
                when (requested.optString(i).lowercase()) {
                    "text" -> com.i.app.ui.components.QuickTestKind.TEXT
                    "speechout", "speech_out", "tts" -> com.i.app.ui.components.QuickTestKind.SPEECH_OUT
                    "transcription", "asr" -> com.i.app.ui.components.QuickTestKind.TRANSCRIPTION
                    "imagegen", "image_gen", "image" -> com.i.app.ui.components.QuickTestKind.IMAGE_GEN
                    else -> null
                }
            }
        }
        ...
        for (kind in kinds) {
            ...
            val state = try {
                com.i.app.ui.components.performTest(kind, entry, repo, context)
            } catch (e: Exception) {
                com.i.app.ui.components.QuickTestState.Failure(
            ...
            results.put(JSONObject().apply {
                ...
                when (state) {
                    is com.i.app.ui.components.QuickTestState.TextReply -> {
                        put("status", "ok"); put("detail", state.text.take(400))
                    }
                    is com.i.app.ui.components.QuickTestState.AudioReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "audio ${state.data.size} bytes")
                    }
                    is com.i.app.ui.components.QuickTestState.ImageReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "image ${state.data.size} bytes")
                    }
                    is com.i.app.ui.components.QuickTestState.Failure -> {
