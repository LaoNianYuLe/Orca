...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.orca.app.data.UpdateChecker.check()) {
            is com.orca.app.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.orca.app.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.orca.app.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.orca.app.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.orca.app.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.orca.app.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.orca.app.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.orca.app.data.UpdateChecker.download(context, url)) {
            is com.orca.app.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.orca.app.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.orca.app.data.UpdateChecker.canInstall(context)
        return try {
            com.orca.app.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.orca.app.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.orca.app.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.orca.app.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.orca.app.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.orca.app.sandbox.offload.ShizukuOffloadHandler(context)
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.orca.app.sandbox.PRootKernel.resolveHostPath(linuxPath)
    ...
    private fun handleIConfigExec(params: JSONObject): JSONObject {
        ...
        return when (sub) {
            ...
            "audit-list" -> {
                ...
                com.orca.app.config.ConfigBridge.auditList(limit, scope)
            }
