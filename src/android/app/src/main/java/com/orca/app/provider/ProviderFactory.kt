...
import com.orca.app.provider.anthropic.AnthropicProvider
import com.orca.app.provider.gemini.GeminiProvider
import com.orca.app.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.orca.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.orca.app.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.orca.app.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.orca.app.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
                    )
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.orca.app.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.orca.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.orca.app.auth.KimiOAuthManager(context, instance.id)
