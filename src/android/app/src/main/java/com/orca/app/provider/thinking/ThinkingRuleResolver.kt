package com.orca.app.provider.thinking
...
data class ThinkingResolveContext(
    ...
    /**
     * [I#163] The catalog affirmatively declares this model has NO effort
...
/**
 * Why a particular wire shape was chosen. Design §8 / GH I#100: the resolved
...
object ThinkingRuleResolver {
    ...
    fun builtInRules(ctx: ThinkingResolveContext): List<ThinkingRule> = buildList {
        // Mistral — GH I#87 / iOS 4592ca9b / 29065ca0. Total prohibition: the
    ...
    fun geminiThinkingConfig(modelId: String, level: ThinkingLevel): JSONObject? {
        // [T-gemini-tts-thinking-400 / I#226] Specialized modalities take
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.orca.app.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.orca.app.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.orca.app.provider.anthropic.AnthropicProvider
