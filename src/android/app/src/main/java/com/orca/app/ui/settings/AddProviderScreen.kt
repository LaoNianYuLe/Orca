...
import com.orca.app.auth.OpenAIOAuthManager
import com.orca.app.auth.OpenRouterOAuthManager
...
import com.orca.app.provider.catalog.ProviderCatalog
...
fun AddProviderScreen(
    ...
    var selectedVoiceTemplate by remember {
        mutableStateOf<com.orca.app.data.model.VoiceProviderTemplate?>(null)
    }
...
private fun ChooseProviderScreen(
    ...
    onSelectVoiceTemplate: (com.orca.app.data.model.VoiceProviderTemplate) -> Unit = {},
) {
    SettingsScaffold(
            ...
            ProviderCatalog.builtIn.forEachIndexed { index, preset ->
                ...
                SettingsRow(
                    ...
                    subtitle = when (preset.protocol) {
                        // The transport is OpenAI-compatible, but the row is
                        // a vendor catalog. Showing only the protocol made
                        // every vendor look like OpenAI in the provider list.
                        ProviderProtocol.OPENAI_COMPATIBLE -> "${preset.displayName} API · API Key"
                    ...
                    icon = icon,
                    iconColor = iconColor,
                    leadingContent = { ProviderBrandBadge(preset.id, preset.displayName) },
        ...
        val templates = com.orca.app.data.model.VoiceProviderTemplate.all
        ...
        SettingsSection(
            ...
            templates.forEachIndexed { index, template ->
                val capabilityRes = when (template.capability) {
                    com.orca.app.data.model.VoiceProviderTemplate.Capability.TTS ->
                        R.string.add_provider_voice_capability_tts
                    com.orca.app.data.model.VoiceProviderTemplate.Capability.ASR ->
                        R.string.add_provider_voice_capability_asr
                    com.orca.app.data.model.VoiceProviderTemplate.Capability.BOTH ->
...
private fun ConfigureProviderScreen(
    ...
    voiceTemplate: com.orca.app.data.model.VoiceProviderTemplate? = null,
    onBack: () -> Unit,
    onSaved: () -> Unit,
) {
...
private fun ColumnScope.ApiKeyConfigSection(
    ...
    if (providerType != ProviderType.openRouter) {
        val defaultUrl = when {
            // A catalog preset is a distinct vendor even when its transport
            // is OpenAI-compatible. Never show OpenAI's endpoint as the
            // fallback for a different vendor.
            preset?.defaultBaseUrl != null -> preset.defaultBaseUrl
            preset != null -> "请填写 ${preset.displayName} 的 API 地址"
            else -> when (providerType) {
    ...
    IButton(
        ...
        enabled = apiKey.isNotBlank() && (preset?.defaultBaseUrl != null || preset == null || customBaseURL.isNotBlank()) || (
...
private fun ColumnScope.OAuthConfigSection(
    ...
    var kimiDeviceAuth by remember {
        mutableStateOf<com.orca.app.auth.KimiDeviceFlow.DeviceAuthorization?>(null)
    }
                            ...
                            try {
                                when (providerType) {
                                    ProviderType.kimiCode -> {
                                        ...
                                        val key = com.orca.app.auth.KimiOAuthManager.login(
                                    ...
                                    ProviderType.anthropic -> {
                                        val key = com.orca.app.auth.ClaudeOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    ProviderType.openAI -> {
                                    ...
                                    ProviderType.xAI -> {
                                        val key = com.orca.app.auth.XAIOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    else -> {
                                ...
                                // signal.
                                errorMessage = if (e is com.orca.app.auth.OAuthNetworkUnreachableException) {
