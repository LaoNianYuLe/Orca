...
import com.orca.app.provider.safeOptString
import com.orca.app.sandbox.NativeOffloadHandler
import com.orca.app.sandbox.NativeOffloadRequest
import com.orca.app.sandbox.NativeOffloadResult
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        if (hasAudioInput && provider !is com.orca.app.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.orca.app.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.orca.app.provider.openai.OpenAIProvider)?.let { openAI ->
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.orca.app.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.orca.app.data.model.ImageEndpointMode.auto
            "images_generations", "images-generations", "images-gen" ->
                com.orca.app.data.model.ImageEndpointMode.imagesGenerations
            "chat_completions", "chat-completions", "chat" ->
                com.orca.app.data.model.ImageEndpointMode.chatCompletions
            else -> null
        }
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.orca.app.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        entry: ModelEntry,
        instance: com.orca.app.data.model.ProviderInstance,
        provider: com.orca.app.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.orca.app.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.orca.app.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.orca.app.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.orca.app.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.orca.app.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.orca.app.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.orca.app.data.model.LLMError.ProviderError ||
                e is com.orca.app.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.orca.app.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.orca.app.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.orca.app.offload.ModelUseManager.entryDict(
            entry,
