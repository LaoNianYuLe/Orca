...
import com.orca.app.shared.TextSegmenter
...
object VoiceCorrection {
    ...
    fun ensureInitialized(context: Context) {
        ...
        // OrcaApp — AppDatabase.getInstance is already the shared handle.
        val dao = runCatching {
            com.orca.app.data.db.AppDatabase.getInstance(app).chatDao()
        }.getOrNull()
        if (dao != null) {
    ...
    fun buildConversationContext(
        context: Context,
        messages: List<com.orca.app.ui.chat.ChatMessage>,
    ): ConversationContext {
        ensureInitialized(context)
