...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.orca.app.data.repository.EnvVarRepository? = null,
) {
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.orca.app.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.orca.app.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.orca.app.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.orca.app.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.orca.app.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.orca.app.mcp.oauth.MCPOAuthConfig(
                    ...
                    if (oauthAuthorized) {
                        OrcaTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.orca.app.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.orca.app.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.orca.app.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.orca.app.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.orca.app.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.orca.app.mcp.oauth.MCPOAuthController.Result.Failed -> {
