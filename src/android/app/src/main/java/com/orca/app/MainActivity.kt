...
import com.orca.app.ui.navigation.AppNavigation
import com.orca.app.ui.navigation.Routes
import com.orca.app.ui.navigation.safeNavigate
import com.orca.app.ui.shell.AppShell
import com.orca.app.ui.settings.KEY_FONT_APP_BASE
import com.orca.app.ui.settings.KEY_KEEP_SCREEN_AWAKE
import com.orca.app.ui.settings.KEY_LANGUAGE
import com.orca.app.ui.settings.KEY_LAUNCH_SESSION
import com.orca.app.ui.settings.KEY_THEME_MODE
import com.orca.app.ui.settings.PREF_APPEARANCE
...
class MainActivity : ComponentActivity() {
     ...
     * [com.orca.app.ui.navigation.AppNavigation]'s LaunchedEffect that
    ...
    override fun attachBaseContext(newBase: android.content.Context) {
        super.attachBaseContext(com.orca.app.i18n.LocaleWrap.wrap(newBase))
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (iApp == null || !iApp.subsystemsInitialized) {
            android.util.Log.w(
                ...
                "app subsystems not initialized (safeMode=" +
                    "${com.orca.app.crash.CrashFrequencyDetector.isSafeMode()}) — " +
                    "showing crash share dialog and finishing",
            )
            com.orca.app.crash.CrashFrequencyDetector.maybeShowOnActivity(
                activity = this,
                ...
                // tapped, reading as "I won't open at all". The process
        ...
        restoredChatSessionId = savedInstanceState?.getString(KEY_CURRENT_CHAT_SESSION_ID)
            ?.takeUnless {
                com.orca.app.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(this)
            }
        ...
        com.orca.app.share.ShareCoordinator.processPendingShare(this)
        ...
        if (BuildConfig.DEBUG) {
            com.orca.app.debug.DebugRPCHandler.currentActivity = java.lang.ref.WeakReference(this)
        }
        ...
        setContent {
            ...
            ITheme(darkTheme = darkTheme, fontScale = fontScale) {
                ...
                AppShell(
                    sessions = app.chatRepository.observeSessions(),
                    onNewChat = {
                    ...
                    showMenuButton = !com.orca.app.ui.navigation.Routes.isSettingsRoute(currentRoute),
                ) {
                    AppNavigation(
     ...
     * is already covered by [com.orca.app.ui.navigation.AppNavigation]
