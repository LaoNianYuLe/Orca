...
/**
 * ITextKit — a self-contained text selection layer that survives
 ...
 *
 * ITextKit avoids that by keeping the *authoritative selection state*
 ...
 * [IMarkdownView] and consumed by [StreamingMarkdownText] via
 * [LocalISelectionController].
 */
...
class SelectionController {
     ...
     * (e.g. a user message bubble, which intentionally skips ITextKit
    ...
    private fun spliceNonShardSpan(
        ...
        // composables (no ITextKit shard), so the per-shard walk drops them
     ...
     * markdown slice contain a block that doesn't register as a ITextKit
 ...
 */
val LocalISelectionController = compositionLocalOf<SelectionController?> { null }
...
fun ProvideSelectionController(
    ...
    CompositionLocalProvider(LocalISelectionController provides controller) {
        content()
    }
}
...
fun RegisterSelectionShard(shard: TextShard?) {
    val controller = LocalISelectionController.current ?: return
