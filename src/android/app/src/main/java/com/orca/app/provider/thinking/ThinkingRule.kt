...
data class ThinkingRule(
     ...
     * persisted UUID; built-ins get a deteritic id that INCLUDES THE SCOPE.
     *
    ...
    /** Deteritic id for built-ins (scope-inclusive); custom rules keep their own. */
    val stableId: String
        get() {
 ...
 * GH I#22 be fixed on the OpenAI path while #70 stayed broken on the Anthropic path.
 */
data class ReasoningEchoPolicy(
    /**
     * `reasoning_content` / `reasoning` / `reasoning_text` — the three spellings observed
     * in the wild, sometimes three different ones on one gateway (GH I#171).
     */
