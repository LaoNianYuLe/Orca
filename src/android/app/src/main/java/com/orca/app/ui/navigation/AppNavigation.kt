...
import com.orca.app.deeplink.DeepLinkCoordinator
import com.orca.app.ui.settings.KEY_LAUNCH_SESSION
...
import com.orca.app.ui.chat.ChatScreen
import com.orca.app.ui.sessions.SessionListScreen
import com.orca.app.ui.settings.AboutScreen
import com.orca.app.ui.settings.AddAgentLoopGroupsScreen
import com.orca.app.ui.settings.AddAgentLoopModelsScreen
import com.orca.app.ui.settings.AddCustomModelScreen
import com.orca.app.ui.settings.BackgroundSettingsScreen
import com.orca.app.ui.settings.AddModelsToGroupScreen
import com.orca.app.ui.settings.ShadowVoiceDetailScreen
import com.orca.app.ui.settings.AddProviderScreen
import com.orca.app.ui.settings.ModelEntryDetailScreen
import com.orca.app.ui.settings.ModelGroupDetailScreen
import com.orca.app.ui.settings.ModelGroupsScreen
import com.orca.app.ui.settings.ProviderDetailScreen
import com.orca.app.ui.settings.ProviderListScreen
import com.orca.app.ui.sandbox.FileBrowserScreen
import com.orca.app.ui.sandbox.FileBrowserViewModel
import com.orca.app.ui.sandbox.FileItem
import com.orca.app.ui.sandbox.FilePreviewScreen
import com.orca.app.ui.sandbox.RootfsManagementScreen
import com.orca.app.ui.settings.EnvironmentVariablesScreen
import com.orca.app.ui.settings.AppearanceScreen
import com.orca.app.ui.settings.SettingsScreen
import com.orca.app.ui.settings.SystemPermissionsScreen
import com.orca.app.ui.settings.SessionStorageDetailScreen
import com.orca.app.ui.settings.SkillDetailScreen
...
import com.orca.app.ui.settings.SharedFoldersScreen
import com.orca.app.ui.settings.SkillsManagementScreen
import com.orca.app.data.repository.EnvVarRepository
import com.orca.app.data.repository.MemoryRepository
import com.orca.app.data.repository.SkillRepository
import com.orca.app.ui.settings.LogDetailScreen
import com.orca.app.ui.settings.LogManagementScreen
import com.orca.app.ui.settings.MemoryFileEditScreen
import com.orca.app.ui.settings.MemoryManagementScreen
import com.orca.app.ui.settings.OffloadPermissionScreen
import com.orca.app.ui.settings.ShizukuPermissionScreen
import com.orca.app.sandbox.RootfsManager
import com.orca.app.sandbox.TerminalSession
import com.orca.app.ui.terminal.TerminalScreen
import com.orca.app.ui.onboarding.OnboardingModelSelectionScreen
import com.orca.app.ui.workspace.CopywritingScreen
import com.orca.app.ui.workspace.ProjectsScreen
...
object Routes {
    ...
    const val COPYWRITING = "copywriting"
    ...
    const val ORCA_SKILLS_BROWSER = "orca_skills_browser"
    ...
    fun isSettingsRoute(route: String?): Boolean {
        ...
        val root = route.substringBefore('?').substringBefore('/')
        return root in SETTINGS_ROUTE_ROOTS
    }

    private val SETTINGS_ROUTE_ROOTS = setOf(
        "settings", "providers", "add_provider", "provider", "voice_service",
        "model_groups", "model_group", "add_models_to_group",
        "add_models_to_agent_loop", "add_groups_to_agent_loop", "model_entry",
        "add_custom_model", "storage", "session_storage", "rootfs_management",
        "mirror_category", "file_browser", "file_preview", "env_vars",
        "skills", "skill", "skill_file", "orca_skills_browser",
        "chat_files", "memory", "mcp", "soul", "memory_file",
        "permissions", "shizuku", "system_permissions", "usage_stats",
        "logs", "log_detail", "appearance", "background", "about",
        "mounted_folders", "mounted_folders_detail",
        "shared_folders", "shared_folders_detail",
        "scheduled_tasks", "onboarding_models",
        "projects", "copywriting", "terminal",
    )
...
fun AppNavigation(
    ...
    LaunchedEffect(Unit) {
        ...
        val hasPendingShare =
            com.orca.app.share.ShareCoordinator.bufferVersion.value > 0
        ...
        val forceHome =
            com.orca.app.diagnostics.HangDetector.shouldForceHomeOnLaunch(context) ||
            com.orca.app.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(context) ||
            com.orca.app.diagnostics.LaunchCycleBeacon.shouldForceHomeOnLaunch()
        ...
        if (target != null) {
            ...
            // the deteritic startup dispatcher, the start destination
    ...
    NavHost(
        ...
        composable(Routes.PROJECTS) {
            ProjectsScreen(
                ...
                onNewChatInProject = { folderId ->
                    navController.safeNavigate(Routes.chat("__new__${java.util.UUID.randomUUID()}__fld__$folderId"))
                },
            )
        }

        composable(Routes.COPYWRITING) {
            CopywritingScreen(
                chatRepository = chatRepository,
                onBack = { navController.safePopBackStack() },
        ...
        composable(
            ...
            SharedFolderDetailScreen(
                ...
                onBrowseFiles = {
                    ...
                    val hostPath = java.io.File(rootfs.rootfsDir, "var/orca/$folderId")
                    val label = when (folderId) {
                        "shared" -> ctx.getString(com.orca.app.R.string.shared_folder_name_shared)
                        "skills" -> ctx.getString(com.orca.app.R.string.shared_folder_name_skills)
                        "memory" -> ctx.getString(com.orca.app.R.string.shared_folder_name_memory)
                        else -> folderId
                    }
        ...
        composable(
            ...
            val category = com.orca.app.ui.sandbox.MirrorCatalog.categoryFromKey(categoryKey)
                ?: return@composable
            com.orca.app.ui.sandbox.MirrorCategoryDetailScreen(
                category = category,
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(
            ...
            val context = androidx.compose.ui.platform.LocalContext.current
            val rootfs = RootfsManager.getInstance(context.applicationContext)
            val varOrca = java.io.File(rootfs.rootfsDir, "var/orca")
            val sessionId = backStackEntry.arguments?.getString("sessionId") ?: return@composable
            val vm = remember(rootfs.rootfsDir.absolutePath, varOrca.absolutePath, sessionId) {
                FileBrowserViewModel(
                    rootPath = rootfs.rootfsDir,
                    initialPath = varOrca.takeIf { it.exists() },
                    rootLabel = "/",
                    ...
                    // inside Alpine's var/orca/ — every subdir reads as
        ...
        composable(Routes.SKILLS) {
            if (skillRepository != null) {
                SkillsManagementScreen(
                    ...
                    onISkillsClick = { navController.safeNavigate(Routes.ORCA_SKILLS_BROWSER) },
                )
            }
        }
        ...
        composable(Routes.ORCA_SKILLS_BROWSER) {
            if (skillRepository != null) {
        ...
        composable(Routes.MCP) {
            if (mcpRepository != null) {
                com.orca.app.ui.settings.MCPIntegrationsScreen(
                    mcpRepository = mcpRepository,
                    onBack = { navController.safePopBackStack() },
                    envVarRepository = envVarRepository,
                )
        ...
        composable(Routes.SOUL) {
            com.orca.app.ui.settings.SoulSettingsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(Routes.SCHEDULED_TASKS) {
            com.orca.app.ui.scheduled.ScheduledTasksScreen(
                onBack = { navController.safePopBackStack() },
                onEditTask = { taskId ->
        ...
        composable(
            ...
            com.orca.app.ui.scheduled.ScheduledTaskRunsScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
        ...
        composable(
            ...
            com.orca.app.ui.scheduled.ScheduledTaskEditScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
