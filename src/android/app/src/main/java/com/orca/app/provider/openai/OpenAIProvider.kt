package com.orca.app.provider.openai
...
import com.orca.app.data.model.LLMMediaAttachment
import com.orca.app.data.model.LLMMessage
import com.orca.app.data.model.LLMModel
import com.orca.app.data.model.LLMResponse
...
import com.orca.app.provider.thinking.ThinkingResolveContext
import com.orca.app.provider.thinking.ThinkingRuleResolver
import com.orca.app.provider.LLMProvider
import com.orca.app.provider.applyUserAgentOverride
...
import com.orca.app.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.orca.app.provider.thinking.ThinkingRuleResolver]'s
    ...
    // i-2026-05-27.log around 13:28 — 3:00 of silence then trip).
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.orca.app.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    /**
     * [I#191] OpenRouter does NOT enable Anthropic prompt caching
    ...
    /**
     * [T-android-mistral-reasoning-422] (GH I#87, iOS 29065ca0)
    ...
    /**
     * [I#163] Talking to xAI's own API (api.x.ai), as opposed to a relay
     ...
     *   • Venice.ai (`api.venice.ai`) — [I#86] resells deepseek / claude /
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.orca.app.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.orca.app.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.orca.app.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.orca.app.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.orca.app.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.orca.app.BuildConfig.DEBUG) {
                com.orca.app.debug.LLMRequestLog.add(
                    com.orca.app.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.orca.app.logging.AppLogger.debug(
                                "OpenAIProvider",
    ...
    private fun buildRequestBody(
        ...
        // I#87) — the request-parameter half of that fix was never
        ...
        // [I#191] Opt this request into Anthropic prompt caching.
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.orca.app.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private suspend fun buildRequest(bodyStr: String): Request {
        ...
        if (isOAuth && !forceChatCompletions) {
            ...
            // I UA here: the ChatGPT OAuth backend validates the
    ...
    private fun injectThinkingParams(body: JSONObject, level: ThinkingLevel, maxTokens: Int) {
        ...
        val ctx = ThinkingResolveContext(
            ...
            // [I#163] null (catalog silent) must read as false here —
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.orca.app.provider.ImageBudget.compressUnderBudget(data)
    ...
    private fun buildResponsesAPIBody(
        ...
        when {
            ...
            // GH I#87). The gate added alongside injectThinkingParams
        ...
        if (com.orca.app.data.FastModePrefs.isEnabled() &&
                ...
                when (msg.role) {
                    LLMMessage.Role.ASSISTANT -> {
                        ...
                        for (tu in msg.contentParts.filterIsInstance<AgentContentPart.ToolUse>()) {
                            ...
                            // generate a deteritic synthetic so the API still accepts it.
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.orca.app.logging.AppLogger.error(
                                        "OpenAIProvider",
     ...
     * it with a deteritic SHA-256-derived id. Deterim matters: the SAME
