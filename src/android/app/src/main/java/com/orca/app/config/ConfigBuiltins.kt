package com.orca.app.config

import android.content.Context
import com.orca.app.config.collections.EnvVarsCollection
import com.orca.app.service.DynamicIslandSupport
import com.orca.app.config.collections.GroupsCollection
import com.orca.app.config.collections.ModelsCollection
import com.orca.app.config.collections.ProvidersCollection
import com.orca.app.config.fields.ClosureField
import com.orca.app.config.fields.PrefsBoolField
import com.orca.app.config.fields.PrefsDoubleField
import com.orca.app.config.fields.PrefsEnumField
import com.orca.app.config.fields.PrefsIntCodedEnumField
import com.orca.app.config.fields.PrefsIntField
import com.orca.app.config.fields.PrefsLongField
import com.orca.app.config.fields.PrefsStringField
import com.orca.app.config.fields.ReadOnlyField
...
internal object ConfigBuiltins {
    ...
    //
    // [T-android-memory-enabled-iconfig] `memory.enabled` mirrors the
    // Settings → Memory switch (MemoryManagementScreen, backed by
    // MemoryGlobalPrefs: SharedPreferences `i_memory_prefs`, key
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.orca.app.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            com.orca.app.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = com.orca.app.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.orca.app.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.orca.app.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.orca.app.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.orca.app.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.orca.app.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(com.orca.app.config.collections.ThinkingRulesCollection(providerRepo))
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.orca.app.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.orca.app.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.orca.app.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.orca.app.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.orca.app.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
                ...
                writer = { v ->
                    ...
                    ids.filterNot { it in valid }.forEach { bad ->
                        com.orca.app.logging.AppLogger.warning(
                            "IConfig", "skipped unknown agentLoopEntries id: $bad"
                        )
                    }
        ...
        r.register(
            ClosureField(
                ...
                writer = { v ->
                    ...
                    ids.filterNot { it in valid }.forEach { bad ->
                        com.orca.app.logging.AppLogger.warning(
                            "IConfig", "skipped unknown agentLoopGroups id: $bad"
                        )
                    }
    ...
    //
    // [T-soul-md-iconfig] Exposes the editable subset of SOUL.md
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.orca.app.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): com.orca.app.agent.SoulFile =
            com.orca.app.agent.SoulStore.load(context)
                ?: com.orca.app.agent.SoulMDParser.parse(
                    com.orca.app.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.orca.app.agent.SoulFile) {
            com.orca.app.agent.SoulStore.save(context, file)
        }
        ...
        r.register(
            ClosureField(
                ...
                description = "The agent's name as shown in chat bubble headers and as the system-prompt identity. Trimmed; empty falls back to \"I\".",
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.orca.app.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.orca.app.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.orca.app.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.orca.app.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.orca.app.agent.SystemPromptBuilder
                    ...
                    val check = com.orca.app.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.orca.app.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.orca.app.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.orca.app.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.orca.app.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.orca.app.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
