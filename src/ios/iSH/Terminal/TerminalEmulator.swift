...
final class TerminalEmulator: ObservableObject {
    ...
    private func handleOSC(command: Int, payload: String) {
        ...
        case 2: // Set Window Title
            title = payload
        case 1337: // iTerm2 proprietary — used by /usr/local/bin/i-open
            // Payload looks like `IOpenURL=https://example.com`. Forward
            // the URL to IOpenURLBroker so the host can present the in-app
    ...
    private func handleITermOSC(payload: String) {
        // Payload is `key=value` — we only recognise `IOpenURL=<url>`,
        // emitted by `/usr/local/bin/i-open` in the rootfs overlay.
        guard let eq = payload.firstIndex(of: "="),
              payload[..<eq] == "IOpenURL" else { return }
        ...
        Task { @MainActor in
            guard IOpenURLBroker.isSupportedScheme(url.scheme) else { return }
            IOpenURLBroker.shared.offer(url)
        }
    }
