...
final class DebugJSONRPC: @unchecked Sendable {
    ...
    private func dispatch(method: String, params: [String: Any]) async throws -> Any {
        ...
        // [T-config-debug-rpc] i-config, driven remotely through the same
    ...
    private func handleVoicePanel(params: [String: Any]) async throws -> Any {
        ...
        case "mic":
            await MainActor.run {
                NotificationCenter.default.post(name: Notification.Name("IDebugVoiceMicTap"), object: nil)
            }
        default:
    ...
    private func handleSyncCKQuery(params: [String: Any]) async -> Any {
        ...
        let zoneName = (params["zone"] as? String) ?? "i-shared"
        ...
        let container = CKContainer(identifier: "iCloud.com.i.app")
        do {
    ...
    /// (i-shared / i-devices / i-secrets) when migration
    ...
    private func handleSyncZoneStats(params: [String: Any]) async -> Any {
        let containerId = "iCloud.com.i.app"
        ...
        if let z = requestedZone {
            ...
            zonesToScan = [
                ("i-shared", ["SessionV2", "MessageV2", "CompactMarkerV2", "SessionFileV2", "SkillV2"]),
                ("i-devices", ["SyncDeviceV2"]),
                ("i-secrets", ["ProviderConfigV2", "EnvVarV2", "EnvVarItem"]),
    ...
    private func handleShellExecute(params: [String: Any]) async throws -> Any {
        ...
        // fs_context, so /var/i/{offloads,attachments,workspace,browser}
        // route to that session's bucket. Omit (or pass nil) for the default
        // global view (fs_context = 0).
        let sessionId = params["sessionId"] as? String
        let fsContext: UInt64 = sessionId.map { IFsRouter.shared.context(for: $0) } ?? 0
    ...
    private func handleTap(params: [String: Any]) async throws -> Any {
        ...
        // IApp's app-embedded synthesis can't reach. iCTRL's isLoopback
    ...
    /// installed XCTest automation service (bundle com.i.ictrl,
    ...
    static func forwardTapToICTRL(params: [String: Any]) async -> [String: Any]? {
        ...
        // SpringBoard if none). Since debug.tap is called BY IApp asking
        // to tap something in ITSELF, pin the query to our own bundle id
        // unless the caller explicitly overrides via ictrl_bundle_id.
        let bundleId = (params["ictrl_bundle_id"] as? String) ?? Bundle.main.bundleIdentifier ?? "com.i.app"
    ...
    private static func activateCellOrDescendant(_ root: UIView) -> String? {
        ...
        // invocation under ICatchObjCException and treat a raise as
        ...
        func attempt(_ label: String, _ body: () -> Bool) -> Bool {
            var accepted = false
            var reason: NSString?
            let ok = ICatchObjCException({ accepted = body() }, &reason)
            if !ok {
                caughtReason = reason
    ...
    private func performTargetTap(strategy: String, lookup: () -> DebugViewInspector.TapTarget?) -> [String: Any] {
        ...
        case .control(let control):
            ...
            if !ICatchObjCException({ control.sendActions(for: .touchUpInside) }, &sendReason) {
    ...
    /// can page through a 9700pt+ cell one screen at a time deteritically.
    ...
    private func knownDatabases() -> [(name: String, url: URL)] {
        // Every app SQLite db lives under Library/IChat/ (verified against
        ...
        let fm = FileManager.default
        let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let chatBase = library.appendingPathComponent("IChat", isDirectory: true)
        let rootfs = RootfsManager.shared.rootfsPath
        return [
            ("i", chatBase.appendingPathComponent("i.db")),
            ("skills", chatBase.appendingPathComponent("i").appendingPathComponent("skills.db")),
