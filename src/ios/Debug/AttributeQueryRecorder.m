...
#define DEFINE_HOOK(NAME, ORIG_VAR) \
    static NSObject *i_attrQuery_##NAME(id self, SEL _cmd, NSAttributedStringKey name, NSUInteger index, NSRangePointer range) { \
...
+ (void)install {
    ...
    dispatch_once(&once, ^{
        ...
        gOrigConcrete = swizzleOne(concrete, (IMP)i_attrQuery_Concrete);
        gOrigMutable  = swizzleOne(mutable,  (IMP)i_attrQuery_Mutable);
        gOrigBase     = swizzleOne(base,     (IMP)i_attrQuery_Base);
