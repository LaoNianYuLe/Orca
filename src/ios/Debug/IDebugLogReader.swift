...
/// In-process log reader for the `i-debug logs` CLI subcommand.
///
/// Unlike the rest of the i-debug CLI (which routes JSON-RPC through
...
///   2. `LoggingManager`'s daily file (`i-YYYY-MM-DD.log`) — the same NSLog
...
/// (T-ios-i-debug-logs-oslogstore).
@objc public final class IDebugLogReader: NSObject {

    @objc(sharedInstance)
    public static let shared = IDebugLogReader()
