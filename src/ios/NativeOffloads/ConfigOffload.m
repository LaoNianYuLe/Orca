...
//  IApp
//
//  Native offload handler for `i-config`.
...
//  shell wrapper at /usr/local/bin/i-config translates the friendly
...
static NSString *const TOOL_NAME = @"i-config";
...
static NSString *const HELP_TEXT =
    @"i-config - read or change I app settings (logged + revertable)\n"
     "\n"
     "USAGE:\n"
...
static int cmd_set(int argc, char **argv, int stdout_fd, int stderr_fd,
                   BOOL compact, BOOL quiet) {
    // [T-ios-i-config-set-shell-escape] (issue #36) `--file <path>` reads
...
void config_offload_register(void) {
    int err = native_offload_add_handler("i-config", config_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/i-config");
        NSLog(@"NativeOffloads: i-config handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register i-config handler (err=%d)", err);
    }
}
