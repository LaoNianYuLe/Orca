...
static NSString *const HELP_TEXT =
    @"apple-alarm - Set alarms and timers on iOS (AlarmKit, iOS 26+)\n"
     ...
     "\n"
     "DEEP LINK:\n"
     "  i://views/alarm              Open the alarm management page in I\n";
...
static dispatch_queue_t authQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        q = dispatch_queue_create("com.i.alarm.auth", DISPATCH_QUEUE_SERIAL);
    });
    return q;
}
    ...
    if (!authorized) {
        NSString *msg = authError
            ? [NSString stringWithFormat:@"AlarmKit authorization failed: %@. "
                "To grant access, open Settings > Privacy & Security > Alarms and enable I.",
...
static int cmd_set_alarmkit(int argc, char **argv, int stdout_fd,
    ...
    if (@available(iOS 26.0, *)) {
        ...
        data[@"view_url"] = @"i://views/alarm";
        data[@"hint"] = @"Alarm is now visible on the I home screen. Open i://views/alarm to manage alarms.";
...
static int cmd_timer_alarmkit(int argc, char **argv, int stdout_fd,
    ...
    if (@available(iOS 26.0, *)) {
        ...
        data[@"view_url"] = @"i://views/alarm";
        data[@"hint"] = @"Timer is now visible on the I home screen. Open i://views/alarm to manage alarms.";
