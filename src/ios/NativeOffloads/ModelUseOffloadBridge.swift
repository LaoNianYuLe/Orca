...
//  IApp
//
//  Swift bridge for i-model-use offload — lists, searches,
...
@objc public class ModelUseOffloadBridge: NSObject {
    ...
    private static func performRun(modelIdOrName: String,
        ...
        if isStreaming {
            ...
            // instead of through the /var/i/attachments symlink, which
            ...
            if !callerSid.isEmpty {
                attachDir = AIChatViewModel.iAttachmentsPersistentDir(for: callerSid).path
            } else {
                attachDir = RootfsManager.shared.dataPath
                    .appendingPathComponent("var/i/attachments").path
            }
            ...
            for (idx, attachment) in response.mediaAttachments.enumerated() {
                ...
                if idx == 0, let outputHostPath, response.text.isEmpty, outputIsMedia {
                    ...
                    // structured file → /var/i/attachments/.
    ...
    private static func resolveModelEntry(_ idOrName: String, providerFilter: String? = nil) throws -> ModelEntry {
        ...
        if let providerFilter, !providerFilter.isEmpty {
            ...
            if filtered.isEmpty {
                throw ModelUseError.modelNotFound("No provider matches --provider '\(providerFilter)'. Use 'i-model-use list' to see provider labels.")
            }
            ...
            if let text = msg["content"] as? String {
                ...
                for part in parts {
                    ...
                    if partType == "text", let t = part["text"] as? String {
                        ...
                        // now throws on unresolvable URLs (i:// to a
    ...
    private static func buildMediaResult(
        ...
        if !callerSid2.isEmpty {
            attachDir = AIChatViewModel.iAttachmentsPersistentDir(for: callerSid2).path
        } else {
            attachDir = RootfsManager.shared.dataPath
                .appendingPathComponent("var/i/attachments").path
        }
    ...
    ///   - `i://<scope>/<path>` — resolve to host storage via the
    ...
    ///   - `/var/i/<scope>/<path>` — bare Linux paths under
    ///     `/var/i/`, mapped to the same scopes as `i://`.
    ...
    private static func resolveImageURL(_ url: String) throws -> LLMMessage.ImageAttachment {
        ...
        // i:// — route through the in-app i URL resolver so
        // attachments/workspace/shared/etc. all work without the caller
        // knowing host paths.
        if url.hasPrefix("i://") {
            guard let parsed = URL(string: url),
                  let resolved = AIChatViewModel.resolveIURL(parsed) else {
                throw ModelUseError.invalidInput("Could not resolve i:// URL '\(url)' — file not found in any session scope. Try /var/i/<scope>/<path> or file:///<host-path>.")
            }
            guard let data = FileManager.default.contents(atPath: resolved.path) else {
                throw ModelUseError.invalidInput("i:// URL '\(url)' resolved to \(resolved.path) but the file is unreadable")
            }
            let mime = Self.imageMimeForExtension(url)
            return LLMMessage.ImageAttachment(mimeType: mime, data: data)
        }

        // /var/i/<scope>/<path> — Linux bare path equivalent of
        // i://<scope>/<path>. Rewrite to a i:// URL and reuse
        // the same resolver so behavior is identical.
        if url.hasPrefix("/var/i/") {
            let stripped = String(url.dropFirst("/var/i/".count))
            // Split into scope + subpath. The i URL "host" is the
            // first path component.
            let parts = stripped.split(separator: "/", maxSplits: 1).map(String.init)
            guard !parts.isEmpty, !parts[0].isEmpty else {
                throw ModelUseError.invalidInput("/var/i/ path must include a scope (attachments, workspace, shared, etc.): '\(url)'")
            }
            let scope = parts[0]
            let sub = parts.count > 1 ? parts[1] : ""
            let iURLStr = sub.isEmpty ? "i://\(scope)" : "i://\(scope)/\(sub)"
            guard let parsed = URL(string: iURLStr),
                  let resolved = AIChatViewModel.resolveIURL(parsed) else {
        ...
        if url.hasPrefix("http://") || url.hasPrefix("https://") {
            throw ModelUseError.invalidInput("http(s):// image URLs are not supported by i-model-use. Download first with `shell_execute` (curl/wget) into /var/i/workspace/, then reference the local path.")
        }
        ...
        if url.hasPrefix("/") {
            ...
            throw ModelUseError.invalidInput("Image file not found at '\(url)'. For i-scope files prefer /var/i/<scope>/<path> or i://<scope>/<path>.")
        }

        throw ModelUseError.invalidInput("Unsupported image_url '\(url)'. Use a data: URL, file:///host/path, /var/i/<scope>/<path>, or i://<scope>/<path>.")
    }

    /// Read a file given a Linux-side path. Tries the iSH rootfs data
    /// directory first (where bind-mounted /var/i/* and /tmp/* land
    ...
    /// e.g. "/private/var/mobile/.../Documents/alpine-rootfs/data/var/i/foo" → "/var/i/foo"
    ...
    /// [T-model-use-media-path-mismatch] Derive the guest (`/var/i/...`)
    ...
    ///   1. Session-persistent dirs under app Library (`.../i/<sid>/{attachments,
    ...
    ///      via the per-session symlinks `ensureISymlinks` maintains
    ///      (`/var/i/attachments` → `.../<sid>/attachments`, etc.). Map by
    ///      matching the persistent-dir prefix for THIS caller session and
    ///      substituting the corresponding `/var/i/<name>` root, preserving
    ///      any sub-path + filename.
    ///   2. A path already inside rootfs `dataPath` (e.g. a user `--output`
    ///      under `/var/i/...` resolved by the CLI). `toLinuxPath` strips
    ///      it directly.
    ///
    /// The previous code hardcoded `/var/i/attachments/<filename>` in the
    ...
    private static func guestLogicalPath(forSavedFile filePath: String, callerSid: String) -> String {
        ...
        if !callerSid.isEmpty {
            let mappings: [(persistDir: String, linuxDir: String)] = [
                (AIChatViewModel.iAttachmentsPersistentDir(for: callerSid).path,
                 AIChatViewModel.iAttachmentsLinuxDir),
                (AIChatViewModel.iWorkspacePersistentDir(for: callerSid).path,
                 AIChatViewModel.iWorkspaceLinuxDir),
                (AIChatViewModel.iOffloadsPersistentDir(for: callerSid).path,
                 AIChatViewModel.iOffloadsLinuxDir),
            ]
        ...
        return AIChatViewModel.iAttachmentsLinuxDir + "/" + (filePath as NSString).lastPathComponent
    }
...
enum ModelUseError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .modelNotFound(let id):
            return "Model '\(id)' not found. Use 'i-model-use list' to see available models."
        case .invalidInput(let msg):
            return msg
        case .modalityNotSupported(let model, let required, let supported):
            return "Model '\(model)' does not support \(required). Supported modalities: \(supported.joined(separator: ", ")). Use 'i-model-use list' to find a model with the required capability."
        }
    }
}
