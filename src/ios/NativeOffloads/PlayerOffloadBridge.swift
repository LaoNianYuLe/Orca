...
//  using the shared UI components (IAudioPreviewView, IVideoFullscreenPlayer).
//
...
@MainActor @objc public class PlayerOffloadBridge: NSObject {
    ...
    // MARK: - Audio (delegates to GlobalAudioPlayer + IAudioPreviewView)
    ...
    private static func openAudioSession(_ session: PlayerSession, completion: @escaping (NSDictionary?, NSString?) -> Void) {
        ...
        // Present IAudioPreviewView
        presentAudioUI(for: session)
        completion(data, nil)
    }

    // MARK: - Video (own AVPlayer + IVideoFullscreenPlayer)
    ...
    private static func presentAudioUI(for session: PlayerSession) {
        ...
        let view = IAudioPreviewView(fileURL: session.fileURL)
    ...
    private static func presentVideoUI(for session: PlayerSession) {
        ...
        let view = IVideoFullscreenPlayer(fileURL: session.fileURL, externalPlayer: player)
        let hostingVC = UIHostingController(rootView: view)
