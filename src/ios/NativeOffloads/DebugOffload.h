...
//  IApp
//
//  Native offload handler for `i-debug`. RPC-backed subcommands are
...
//  and is available in ALL builds (T-ios-i-debug-logs-oslogstore), so the
...
/// Register the i-debug native handler. Registered in every build so the
