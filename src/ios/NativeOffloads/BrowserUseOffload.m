...
//  IApp
//
//  Native offload handler for `i-browser-use`.
...
static NSString *const TOOL_NAME = @"i-browser-use";
...
static NSString *const HELP_TEXT =
    @"i-browser-use - Drive the in-app WebView from the shell\n"
     ...
     "                   /var/i/browser/ and referenced via image_path +\n"
     "                   i_url, so shells don't get flooded with base64.\n"
     ...
     "                      /var/i/browser/screenshot_<ms>.jpg\n"
     ...
     "  i-browser-use screenshot --full-page\n"
...
static NSDictionary *buildInputJson(int argc, char **argv, NSString **errOut) {
    ...
    // (mirrors `i-config set --file`). Parse failures are surfaced as an
...
void browser_use_offload_register(void) {
    int err = native_offload_add_handler("i-browser-use", browser_use_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/i-browser-use");
        NSLog(@"NativeOffloads: i-browser-use handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register i-browser-use handler (err=%d)", err);
    }
}
