...
final class LoggingManager: ObservableObject {
    ...
    private func safeWrite(_ data: Data) -> Bool {
        ...
        var reason: NSString?
        let ok = IFileHandleSafeWrite(handle, data, &reason)
        if !ok {
    ...
    private func rotateLogFileIfNeeded() {
        ...
        let fileURL = logDirectory.appendingPathComponent("i-\(today).log")
    ...
    /// pattern (`i-YYYY-MM-DD.log`, `crash-YYYYMMDD-HHmmss.log`) — the
    ...
    private static func logDate(of url: URL) -> Date? {
        ...
        if name.hasPrefix("i-") {
            ...
            if let d = df.date(from: String(name.dropFirst("i-".count))) { return d }
    ...
    /// `"running"` for `i-*.log`, `"crash"` for `crash-*.log` (written by
    ...
    func deleteLog(at url: URL) {
        ...
        writerQueue.async { [weak self] in
            ...
            if url.lastPathComponent == "i-\(today).log" {
                self.logFileHandle?.closeFile()
                self.logFileHandle = nil
                self.currentLogDate = ""
            }
