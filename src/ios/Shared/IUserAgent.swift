...
///
/// Without an explicit header, URLSession sends `I/<CFBundleVersion>
/// CFNetwork/… Darwin/…` — i.e. the **build number** (e.g. `I/1`), not the
...
enum IUserAgent {
    ...
    static var `default`: String {
        ...
        let ua = "I/\(version) (iOS \(osVersion); \(model))"
        cached = ua
        return ua
    }
}
