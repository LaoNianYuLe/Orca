...
final class EnvVarStore: ObservableObject {
    ...
    nonisolated private static let keychainService = "com.i.app.envvar"
    ...
    /// Keychain-stored value. Used by i-config so the agent can
    ...
    nonisolated func allAsDict() -> [String: String] {
        ...
        let libraryURL = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let fileURL = libraryURL.appendingPathComponent("IChat/env-vars.json")
