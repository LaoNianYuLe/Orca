...
enum ConfigAuditActor: String, Codable {
    /// LLM agent invoked `i-config <topic> set …`.
    case agent
    ...
    /// e.g. toggling the master `permissions.iConfig.enabled` switch).
    case user
    /// Agent invoked `i-config audit revert <id>`.
...
struct ConfigAuditEntry: Identifiable, Codable, Hashable {
    ...
    let revertOf: String?
    /// Caller-supplied description (`i-config set --caption "..."`),
