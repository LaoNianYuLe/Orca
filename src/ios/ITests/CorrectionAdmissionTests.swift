...
final class CorrectionAdmissionTests: XCTestCase {
    ...
    func testLatinPhoneticSimilarity() {
        ...
        // The reported bug: linux→i. Skeletons lnx/mns — same syllable count
        // (2), "n" anchored at index 1, skeleton similarity 0.33 ≥ 0.30.
        XCTAssertTrue(f("linux", "i"))
        ...
        XCTAssertFalse(f("i", "minutes"))   // 2 vs 3 — a rewrite, correctly dropped
    ...
    func testLinuxIAdmitted() {
        // The specific user report, end-to-end through judge().
        XCTAssertEqual(judge("linux", "i"), .latinPhonetic)
    ...
    func testRealDataAdmissionMatrix() {
        let shouldCollect: [(String, String)] = [
            ...
            ("linux", "i"),   // Signal 4 — the GH report; nothing else catches it
        ]
