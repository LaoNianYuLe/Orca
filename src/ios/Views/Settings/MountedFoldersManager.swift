...
//  mounted into /var/i/mounts/<name> via security-scoped bookmarks.
//
//  Lifecycle:
...
//  - ensureISymlinks() creates /var/i/mounts/<name> symlinks in fakefs
//
...
struct MountedFolderEntry: Codable, Identifiable, Equatable {
    ...
    let id: UUID
    /// User-chosen name; becomes the folder name under /var/i/mounts/<name>.
    ...
    /// User intent: does the user *want* I (shell + AI + Browse Files) to
...
final class MountedFoldersManager {
    ...
    /// Mounts metadata lives in the App Group's private IConfig subdir
    /// so it does NOT leak into iOS Files' "On My iPhone → I" view.
    /// Historically stored in `iAppGroupRoot/mounted-folders.json`, but
    /// that path sits inside the FileProvider's exposed root.
    private static var storeURL: URL {
        return AIChatViewModel.iConfigRoot.appendingPathComponent("mounted-folders.json")
    }
    ...
    private static var legacyStoreURL: URL {
        AIChatViewModel.iAppGroupRoot.appendingPathComponent("mounted-folders.json")
    }

    /// Move `mounted-folders.json` out of providerRoot into IConfig the
    ...
    private func migrateStoreFileIfNeeded() {
        ...
        do {
            ...
            mountLog.info("migrateStoreFile: moved \(legacy.lastPathComponent) to IConfig/")
        } catch {
            // Fallback: copy + delete, in case move-across-bind fails.
            do {
                try fm.copyItem(at: legacy, to: current)
                try fm.removeItem(at: legacy)
                mountLog.info("migrateStoreFile: copied \(legacy.lastPathComponent) to IConfig/ (move failed, copy+delete succeeded)")
            } catch {
    ...
    /// I-internal soft lock on top of the OS-level `isWritable`. Calling
    ...
    func pushExternalMountSnapshot() {
        ...
        let snapshot: [ISHExecutionCoordinator.ExternalMountSpec] = entries.compactMap { entry in
            ...
            let linuxDir = "\(AIChatViewModel.iMountsLinuxDir)/\(entry.name)"
    ...
    /// Returns the entry matching a fakefs symlink path (`/var/i/mounts/<name>/...`).
    ...
    func entryForLinuxPath(_ linuxPath: String) -> MountedFolderEntry? {
        let prefix = AIChatViewModel.iMountsLinuxDir + "/"
    ...
    nonisolated static func probeWritable(at url: URL) -> Bool {
        let probe = url.appendingPathComponent(".i-probe-\(UUID().uuidString)")
    ...
    private func removeMountSymlink(name: String) {
        ...
        let linkPath = dataPath
            .appendingPathComponent("var/i/mounts", isDirectory: true)
