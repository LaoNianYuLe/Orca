...
class StorageManagementViewModel: ObservableObject {
    ...
    func load() {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            // Chat database
            let dbURL = libraryURL.appendingPathComponent("IChat/i.db")
            let dbSize = self.fileSize(at: dbURL)

            // Per-session i files: Library/IChat/i/<sessionId>/
            let iBaseURL = libraryURL.appendingPathComponent("IChat/i", isDirectory: true)
            ...
            // Enumerate i directories
            var iSizes: [String: Int64] = [:]
            if let contents = try? self.fm.contentsOfDirectory(at: iBaseURL, includingPropertiesForKeys: nil, options: .skipsHiddenFiles) {
                for dir in contents {
                    let sid = dir.lastPathComponent
                    if sessionIds.contains(sid) {
                        iSizes[sid] = self.directorySize(at: dir)
                    }
                }
            }
...
struct SessionStorageDetailView: View {
    ...
    private var iURL: URL {
        let lib = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        return lib.appendingPathComponent("IChat/i/\(session.id)", isDirectory: true)
    }

    private var totalFileSize: Int64 { currentISize }
    ...
    var body: some View {
        List {
            Section("I Files") {
                if currentISize > 0 {
                    NavigationLink {
                        FileBrowserView(rootPath: iURL)
                    } label: {
                        HStack {
                            Label("Browse Files", systemImage: "folder")
                            Spacer()
                            Text(formatter.string(fromByteCount: currentISize))
                                .foregroundStyle(.secondary)
                        }
                    }
                } else {
    ...
    private func clearFiles() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let iDir = lib.appendingPathComponent("IChat/i/\(sessionId)", isDirectory: true)
            try? fm.removeItem(at: iDir)
