...
struct IVideoPlayerView: View {
    ...
    private func notifyFileReadyOnce() {
        ...
        NotificationCenter.default.post(name: .iAttachmentSizeChanged, object: url.absoluteString)
    }
    ...
    private func loadThumbnail() {
        ...
        if let cached = IMediaCache.shared.thumbnail(for: cacheKey) {
            thumbnail = cached
        ...
        generator.generateCGImagesAsynchronously(
            ...
            if let cgImage {
                ...
                IMediaCache.shared.setThumbnail(img, for: cacheKey)
...
struct IVideoFullscreenPlayer: View {
    let fileURL: URL
    ...
    var body: some View {
        ZStack {
            Color.black.opacity(1.0 - Double(max(dragOffset, 0) / dismissThreshold) * 0.4)
                .ignoresSafeArea()
            if let player {
                IAVPlayerLayerView(player: player)
                    .ignoresSafeArea()
                    .offset(y: dragOffset)
            }
...
struct IAVPlayerLayerView: UIViewRepresentable {
    let player: AVPlayer
