...
private let iLogger = AppLogger(category: "IMedia")
// MARK: - i:// Media Provider

// MARK: Media Cache — avoids re-loading/decoding when LazyVStack recycles cells

final class IMediaCache {
    static let shared = IMediaCache()

    /// Downsampled UIImage cache keyed by absolute i:// URL string.
    private let imageCache = NSCache<NSString, UIImage>()
    /// Video thumbnail cache keyed by file path.
    private let thumbnailCache = NSCache<NSString, UIImage>()
    /// Resolved host file URL cache keyed by i:// URL string.
    ...
    /// time each was taken.
    ///
    /// `iMediaCacheKey` is evaluated inside `body` (it feeds `.task(id:)`),
    /// so it runs on the main actor once per tile per body pass. Its `stat()`
    /// was previously uncached, and for a `i://mounts/<name>/…` source the
    ...
    /// after we write to a i:// path ourselves, where waiting out the TTL
...
/// Resolve a i:// URL with caching to avoid repeated FileManager lookups.
///
/// Cache key is scoped by the active session id because per-session paths
/// (e.g. `i://attachments/foo.png`) resolve to different files in
/// `Library/IChat/i/<sessionId>/…` depending on which session is
...
func resolveIFileURLCached(url: URL) -> URL? {
    let sessionScope = AIChatViewModel.activeSessionId ?? "nil"
    let key = "\(sessionScope)|\(url.absoluteString)"
    if let cached = IMediaCache.shared.resolvedURL(for: key) {
        iLogger.info("[IImage][ResolveCache] HIT url=\(url.absoluteString) → \(cached.path)")
        return cached
    }
    if let resolved = resolveIFileURL(url: url) {
        // Don't persist a mount resolution taken on the main thread: that path
        // is returned unverified (see the mounts branch of resolveIFileURL,
        ...
        if !isUnverifiedMountGuess {
            IMediaCache.shared.setResolvedURL(resolved, for: key)
        }
        iLogger.info("[IImage][ResolveCache] MISS→resolved url=\(url.absoluteString) → \(resolved.path)")
        return resolved
    }
    iLogger.warning("[IImage][ResolveCache] MISS→nil url=\(url.absoluteString)")
    return nil
}
...
///
/// For `i://` sources we resolve to the host file URL and stat it.
...
func iMediaCacheKey(for source: String) -> String {
    guard let url = URL(string: source) else { return source }
    return iMediaCacheKey(for: url)
}

func iMediaCacheKey(for url: URL) -> String {
    let base = url.absoluteString
    // Non-i schemes: return as-is, we don't own that file's lifecycle.
    guard url.scheme == "i" else { return base }
    guard let fileURL = resolveIFileURLCached(url: url) else {
    ...
    // fingerprint when we have one; see IMediaCache.fingerprint(for:).
    if let cached = IMediaCache.shared.fingerprint(for: path) {
        return cached
    }
    ...
    IMediaCache.shared.setFingerprint(key, for: path)
    return key
}
...
let iAudioExtensions: Set<String> = ["mp3", "m4a", "wav", "aac", "ogg", "flac"]
let iVideoExtensions: Set<String> = ["mp4", "mov", "m4v", "avi", "mkv", "webm"]
let iImageExtensions: Set<String> = ["png", "jpg", "jpeg", "gif", "webp", "bmp", "tiff"]
let iTextExtensions: Set<String> = [
...
let iMarkdownExtensions: Set<String> = ["md", "markdown"]
let iHTMLExtensions: Set<String> = ["html", "htm", "svg"]
let iDocumentExtensions: Set<String> = [
    "pdf",
    // Microsoft Office
...
/// Maps directly to persistent storage: Library/IChat/i/<sessionId>/<subdir>/<path>
/// No dependency on iSH boot or bind mounts.
func resolveIFileURL(url: URL) -> URL? {
    guard let host = url.host else {
        iLogger.warning("[ResolveIURL] no host in URL: \(url.absoluteString)")
        return nil
    }
    ...
    if let sid = AIChatViewModel.activeSessionId {
        for subPath in subPaths {
            ...
            iLogger.info("[IImage][Resolve] \(url.absoluteString) → session=\(sid) → \(persistURL.path) exists=\(exists) size=\(fileSize)")
    ...
    for (subdir, globalDir) in globalDirs where host == subdir {
        for subPath in subPaths {
            ...
            if fm.fileExists(atPath: candidate.path) {
                iLogger.info("[IImage][Resolve] \(url.absoluteString) → global \(subdir) → \(candidate.path)")
                return candidate
            }
        }
    }

    // User-mounted external folders: i://mounts/<name>/<path>
    ...
    if host == "mounts" {
        for subPath in subPaths {
            ...
            if let mountURL {
                ...
                // `iMediaCacheKey`, so on the main thread we accept the
                ...
                // the precise check.
                if Thread.isMainThread {
                    iLogger.info("[IImage][Resolve] \(url.absoluteString) → mount '\(mountName)' → \(candidate.path) (unverified, main thread)")
                    return candidate
                }
                if fm.fileExists(atPath: candidate.path) {
                    iLogger.info("[IImage][Resolve] \(url.absoluteString) → mount '\(mountName)' → \(candidate.path)")
                    return candidate
                }
            }
        }
    ...
    // [T-ios-iurl-cross-session-isolation] DO NOT scan other sessions.
    // i://attachments/<file> is session-scoped: it must only resolve
    ...
    // fallback was a P0 isolation leak — i-model-use emits fixed names like
    ...
    iLogger.warning("[IImage][Resolve] \(url.absoluteString) → not found in active session, global dirs, or mounts (cross-session scan disabled for isolation)")
    return nil
}

// MARK: - i:// Link Handling

/// SF Symbol icon for a given file extension.
private func iFileIcon(for ext: String) -> String {
...
/// Tappable file chip for i:// links in Markdown.
private struct IFileChipView: View {
    let url: URL
    ...
    var body: some View {
        ...
        Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: iFileIcon(for: ext))
                    .font(.system(size: 13))
...
struct AsyncImageTile: View {
    ...
    var body: some View {
        ...
        // the underlying i:// file invalidates the displayed
        // thumbnail (T-image-cache-mtime-35133). URL string alone would
        // keep the stale image until the cell recycles.
        .task(id: iMediaCacheKey(for: meta.iURL)) { loadThumbnail() }
    }

    private func loadThumbnail() {
        // Fingerprint key captures file size + mtime so a rewrite of the
        // same i:// path invalidates the cache automatically.
        let cacheKey = "attach:\(iMediaCacheKey(for: meta.iURL))"
        if let cached = NativeMediaImageCache.shared.image(for: cacheKey) {
            thumbnail = cached
            return
        }
        guard let url = URL(string: meta.iURL),
              let fileURL = resolveIFileURLCached(url: url) else { return }
...
struct AsyncVideoTile: View {
    ...
    var body: some View {
        Button {
            if let url = URL(string: meta.iURL) { openURL(url) }
        } label: {
...
/// Environment action for handling i:// and http(s):// URL taps.
///
...
private struct IURLKey: EnvironmentKey {
    static let defaultValue = IURLAction { _ in .systemAction }
}

extension EnvironmentValues {
    var openIURL: IURLAction {
        get { self[IURLKey.self] }
        set { self[IURLKey.self] = newValue }
    }
}

/// Thin modifier that overrides `\.openURL` for a subtree and forwards to the
/// top-level `\.openIURL` action. Keeps cell-internal views from holding their
/// own sheet @State (which would break presentation animations).
struct IOpenURLHandler: ViewModifier {
    @Environment(\.openIURL) private var openIURL
...
private struct IImageView: View {
    let url: URL?
    ...
    var body: some View {
        if let url, url.scheme == "i" {
            let ext = url.pathExtension.lowercased()
            let _ = iLogger.info("[IImage][View] render url=\(url.absoluteString) ext=\(ext) hasLoadedImage=\(self.loadedImage != nil) attempt=\(self.loadAttempt)")
            if iAudioExtensions.contains(ext) {
                if let fileURL = resolveIFileURLCached(url: url) {
                    IAudioPlayerView(url: url, fileURL: fileURL)
                }
            } else if iVideoExtensions.contains(ext) {
                if let fileURL = resolveIFileURLCached(url: url) {
                    IVideoPlayerView(url: url, fileURL: fileURL)
                }
            } else {
                iImageContent(url: url)
                    ...
                    // already fingerprint-keyed via iMediaCacheKey.
                    .task(id: iMediaCacheKey(for: url)) {
                        await loadWithRetry(url: url)
                    }
            }
        } else if let url {
            // Fall through to default async loading for non-i URLs
    ...
    @ViewBuilder
    private func iImageContent(url: URL) -> some View {
        let maxH = UIScreen.main.bounds.height / 2
    ...
    private func loadWithRetry(url: URL) async {
        let cacheKey = iMediaCacheKey(for: url)
        iLogger.info("[IImage][MarkdownUI] loadWithRetry START url=\(url.absoluteString)")
        // Check memory cache first — instant on cell recycle
        if let cached = IMediaCache.shared.image(for: cacheKey) {
            iLogger.info("[IImage][MarkdownUI] CACHE HIT url=\(url.absoluteString) size=\(cached.size.width)x\(cached.size.height)")
            loadedImage = cached
        ...
        for attempt in 0..<6 {
            ...
            loadAttempt = attempt
            if let fileURL = resolveIFileURLCached(url: url) {
                iLogger.info("[IImage][MarkdownUI] attempt=\(attempt) url=\(url.absoluteString) resolved=\(fileURL.path) exists=\(FileManager.default.fileExists(atPath: fileURL.path))")
                // Load and downsample on background thread to avoid blocking main thread
                let image = await Task.detached(priority: .userInitiated) {
                    guard let data = try? Data(contentsOf: fileURL) else {
                        iLogger.warning("[IImage][MarkdownUI] Data(contentsOf:) FAILED url=\(url.absoluteString) path=\(fileURL.path)")
                        return nil as UIImage?
                    }
                    let fileSize = data.count
                    let img = downsampleImage(data: data, maxPixelSize: 2048)
                    if let img {
                        iLogger.info("[IImage][MarkdownUI] decoded url=\(url.absoluteString) dataSize=\(fileSize) imgSize=\(img.size.width)x\(img.size.height)")
                    } else {
                        iLogger.warning("[IImage][MarkdownUI] downsample FAILED url=\(url.absoluteString) dataSize=\(fileSize)")
                    }
                    return img
                }.value
                if let image {
                    IMediaCache.shared.setImage(image, for: cacheKey)
                    loadedImage = image
                    iLogger.info("[IImage][MarkdownUI] SUCCESS url=\(url.absoluteString) attempt=\(attempt)")
                    // [T-attachment-size-invalidate] Placeholder (~fixed) → fit (variable, up to half-screen).
                    notifySizeChangeOnce()
                    return
                }
            } else {
                iLogger.warning("[IImage][MarkdownUI] resolve FAILED attempt=\(attempt) url=\(url.absoluteString)")
            }
        }
        iLogger.error("[IImage][MarkdownUI] GAVE UP after 6 attempts url=\(url.absoluteString)")
        loadAttempt = 0
    }

    /// Post .iAttachmentSizeChanged at most once per view lifetime.
    ...
    private func notifySizeChangeOnce() {
        ...
        NotificationCenter.default.post(name: .iAttachmentSizeChanged, object: urlStr)
    }
}
...
// MARK: - i:// Image File Preview (from URL handler)

/// Fullscreen image preview loading from a local file URL.
struct IImageFilePreviewView: View {
    let fileURL: URL
...
/// `.task(id: iMediaCacheKey(...))`; mirror it here with a cheap stat so a
/// same-path rewrite re-reads the file. Unlike iMediaCacheKey this takes the
/// already-resolved disk URL directly (no i:// re-resolve).
...
private struct ITextView: UIViewRepresentable {
    let text: String
...
// Non-private so cross-file preview sheets (e.g. ILinkPreviewView in
...
struct IMarkdownPreviewView: View {
    let fileURL: URL
    @Environment(\.dismiss) private var dismiss
...
struct IHTMLPreviewView: View {
    ...
    var body: some View {
        ...
        // iPad/Mac, matching IMarkdownPreviewView. iPhone unaffected.
        .modifier(WideSheetSizingModifier())
...
// MARK: - i:// Document Preview

import QuickLook

struct IDocumentPreviewView: UIViewControllerRepresentable {
    let fileURL: URL
    ...
    class Coordinator: NSObject, QLPreviewControllerDataSource {
        ...
        /// iMediaCacheKey's stat-based approach.
