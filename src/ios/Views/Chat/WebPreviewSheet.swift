...
final class WebViewHolder: NSObject, ObservableObject {
    ...
    init(url: URL, localFile: Bool) {
        ...
        // BrowserUseManager so i:// HTML opened from chat (which routes
    ...
    /// inside a `.fullScreenCover` (the iSH terminal → `i-open` flow
...
struct WebPreviewMoreMenu: View {
    ...
    /// Only the i:// HTML preview wires this up — it triggers the
