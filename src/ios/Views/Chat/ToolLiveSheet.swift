...
/// environment — callers override that to present ILinkPreviewView.
///
...
fileprivate func attributedShellLine(_ text: String) -> AttributedString {
    ...
    for m in matches {
        ...
        // which matches path fragments in shell output (e.g. "/Users/.../github.com/I/...").
...
struct ToolLiveSheet: View {
    ...
    private func durationLabel(started: Date) -> String {
        ...
        return IStepTimestampFormatter.duration(seconds: secs, stillRunning: stillRunning)
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            IOpenURLBroker.shared.toolSheetVisible = true
        }
        .onDisappear {
            stopBrowserTimer()
            resourceMonitor.stop()
            IOpenURLBroker.shared.toolSheetVisible = false
        }
        // Auto-present an in-app browser preview when a shell tool emits an
        // OSC IOpenURL marker while this sheet is visible.
        //
        // Only intercepts http/https/about: URLs — `i://` chat-resource
        ...
        .onReceive(IOpenURLBroker.shared.$pendingURL.dropFirst().compactMap { $0 }) { url in
            guard IOpenURLBroker.isWebScheme(url.scheme) else { return }
        ...
        .sheet(item: $activeSheet, onDismiss: {
            ...
            case .linkPreview(let url):
                ILinkPreviewView(url: url, browserPool: browserPool)
            }
        }
    }
    ...
    /// - Parameter toolResult: Optional tool result info (i_url etc.) shown below the editor.
                ...
                if dur != nil || block.toolStartTime != nil {
                    VStack(alignment: .trailing, spacing: 1) {
                        ...
                        if let started = block.toolStartTime {
                            Text(IStepTimestampFormatter.string(from: started))
    ...
    private var toolTitle: String {
        switch block.kind {
        case .shellTool: return "I is using Shell"
        case .fileReadTool: return "I is reading File"
        case .fileWriteTool: return "I is using Editor"
        case .fileEditTool: return "I is editing File"
        case .browserTool: return "I is using Browser"
        case .readImageTool: return "I is reading Image"
        case .memoryTool: return "I is using Memory"
        case .info: return "I"
        case .text: return "I"
        case .thinking: return "I"
        }
    }
