...
struct ISHTerminalView: View {
    ...
    /// URL captured from an OSC `IOpenURL` marker emitted by
    /// /usr/local/bin/i-open — presented in an in-app WKWebView sheet.
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            IOpenURLBroker.shared.terminalVisible = true
        }
        .onDisappear {
            IOpenURLBroker.shared.terminalVisible = false
        }
        ...
        // In-app WKWebView preview for URLs emitted by `i-open` via
        // the OSC 1337 IOpenURL marker. `TerminalEmulator` parses the
        // marker and forwards the URL through `IOpenURLBroker`.
        ...
        .onReceive(IOpenURLBroker.shared.$pendingURL.dropFirst().compactMap { $0 }) { url in
            // Only web schemes are routed here — i:// resource
            // previews need AIChatView's `handleIURLTap` and aren't
        ...
        .sheet(item: $linkPreviewURL) { url in
            ...
            // uses — `ILinkPreviewView` with its toolbar (reload /
            ...
            ILinkPreviewView(url: url, browserPool: nil)
        }
    }
}
...
class ISHTerminalViewModel: ObservableObject {
    ...
    func startShell(sessionId: String? = nil, initCommand: String? = nil) {
        ...
        // Mount /var/i/* for this session BEFORE starting the shell.
        ...
        if let sid = sessionId {
            stepStart = CFAbsoluteTimeGetCurrent()
            print("[ISHTerminal] Mounting /var/i for session \(sid) before shell start")
    ...
    private let inputQueue = DispatchQueue(label: "com.i.terminal.input")
