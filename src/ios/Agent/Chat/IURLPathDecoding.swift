...
/// Shared percent-decoding helpers for resolving `i://` URLs to on-disk
/// subpaths.
///
/// A correctly-formed `i_url` percent-encodes the filename exactly once
/// (see `linuxPathToIURL`), so `URL(string:).path` decodes it back to the
...
enum IURLPathDecoding {
    /// Candidate subpaths for a `i://` URL, in priority order.
