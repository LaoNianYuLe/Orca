...
struct FileMentionEntry: Identifiable, Equatable, Hashable {
    ...
    /// iSH-visible path (e.g. `/var/i/workspace/foo/bar.py`). Inserted into input.
    ...
    /// Path shown in the menu (strips the `/var/i/` prefix for density).
...
final class FileMentionIndex: ObservableObject {
    ...
    private var scanQueue = DispatchQueue(label: "com.i.filementionindex", qos: .userInitiated)
    ...
    private static func sessionRoots(sessionId: String?) -> [ScanRoot] {
        ...
        return [
            ScanRoot(
                scope: .workspace,
                hostURL: AIChatViewModel.iWorkspacePersistentDir(for: sid),
                linuxPrefix: AIChatViewModel.iWorkspaceLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .attachments,
                hostURL: AIChatViewModel.iAttachmentsPersistentDir(for: sid),
                linuxPrefix: AIChatViewModel.iAttachmentsLinuxDir,
                mountName: nil
            ),
        ]
    }
    ...
    private static func sharedRoots() -> [ScanRoot] {
        [
            ScanRoot(
                scope: .shared,
                hostURL: AIChatViewModel.iSharedPersistentDir,
                linuxPrefix: AIChatViewModel.iSharedLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .skills,
                hostURL: AIChatViewModel.iSkillsPersistentDir,
                linuxPrefix: AIChatViewModel.iSkillsLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .memory,
                hostURL: AIChatViewModel.iMemoryPersistentDir,
                linuxPrefix: AIChatViewModel.iMemoryLinuxDir,
                mountName: nil
            ),
        ]
    }
    ...
    private static func mountRoots() -> [ScanRoot] {
        ...
        return mgr.entries.compactMap { entry -> ScanRoot? in
            ...
            let linuxPrefix = "\(AIChatViewModel.iMountsLinuxDir)/\(entry.name)"
    ...
    private struct ScanRoot {
        ...
        let hostURL: URL
        /// Linux-visible path prefix (e.g. `/var/i/workspace`).
    ...
    /// substring. Lets `@mounts` match `/var/i/mounts/<name>/...` even
    ...
    /// Is this the immediate child of a scope root (e.g. `/var/i/skills/foo`,
    /// `/var/i/mounts/bar`, `/var/i/shared/baz`)? These top-level
