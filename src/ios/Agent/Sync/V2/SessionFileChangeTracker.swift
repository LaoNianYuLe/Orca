...
actor SessionFileChangeTracker {
    ...
    func recordBatch(
        ...
        for (sid, rawPath, opCode, _) in events {
            ...
            guard let rel = Self.resolveSessionRelative(
                rawPath: rawPath, sessionId: sid, iBaseURL: iBaseURL
            ) else {
                totalIgnored &+= 1
                ignored += 1
    ...
    /// schedule races).
    func drainAllWithMtimeFilter(iBaseURL: URL) -> [String: [String: Change]] {
    ...
    /// file:
    ///
    ///   • Guest path:  `/var/i/<subdir>/<rel>` (fakefs path-normalize
    ///     left the bind-mount symlink alone, e.g. unlink/rename)
    ///   • Host path:   `<iBaseURL>/<sid>/<subdir>/<rel>` (path-normalize
    ...
    nonisolated static func resolveSessionRelative(
        rawPath: String, sessionId: String, iBaseURL: URL
    ) -> String? {
        // Guest-shape: trivial prefix strip.
        let guestPrefix = "/var/i/"
        ...
        // Host-shape: must live under this session's per-session i dir.
...
///
/// iSH stays agnostic about I sessions — this Swift bridge knows the
/// per-session host directory layout (`<iBaseURL>/<sid>/<subdir>/...`)
/// and the bind-mount guest layout (`/var/i/<subdir>/...`), and resolves
...
func installSessionFileChangeTracker(kernel: ISHKernel) {
    ...
    installLogger.info("[SessionFileTracker] installing fakefs change handler — iSH realfs writes/unlinks/renames under /var/i/{workspace,attachments,browser,offloads} (or the equivalent host APFS path) will be queued for iCloud SessionFile sync")
    // Captured at install time; iBaseURL doesn't change for the
    // lifetime of the app process.
    let iBaseURL = ChatStore.shared.iBaseURL
    kernel.installFakefsChangeHandler { events in
        ...
        // the write/delete; IFsRouter maps that back to the sid that
        ...
        for event in events {
            ...
            if event.fsContext != 0 {
                resolvedSid = IFsRouter.shared.sid(for: event.fsContext) ?? fallbackSid
            } else {
        ...
        if !tuples.isEmpty {
            Task.detached(priority: .utility) {
                await SessionFileChangeTracker.shared.recordBatch(tuples, iBaseURL: iBaseURL)
            }
        }

        // Skills side-channel: if any event in this batch touched a
        // path under `/var/i/skills/` (or the equivalent host APFS
        ...
        if FakefsSkillPathCheck.batchTouchesSkills(events: events, iBaseURL: iBaseURL) {
...
fileprivate enum FakefsSkillPathCheck {
    static func batchTouchesSkills(events: [ISHFakefsChangeEvent], iBaseURL: URL) -> Bool {
        let guestPrefix = "/var/i/skills"
        // host APFS path looks like ".../IChat/i/skills/..."
        // — iBaseURL = "<lib>/IChat/i", so the host prefix
        //   to check is "<iBaseURL>/skills".
        let hostPrefix = iBaseURL.standardizedFileURL.path + "/skills"
        for evt in events {
            let p = evt.linuxPath
...
final class SkillFilesystemNotifier {
    ...
    /// True when at least one fakefs event touching /var/i/skills/
