...
final class SyncCore {
    ...
    private func startPathMonitor() {
        ...
        m.start(queue: DispatchQueue(label: "com.i.sync.pathMonitor"))
        pathMonitor = m
    }
    ...
    func sendNow(trigger: SyncSendTrigger) async {
        ...
        // change tracker (shell tools writing under /var/i/<subdir>)
    ...
    /// truncate, unlink, rename inside `/var/i/<workspace|attachments|browser|offloads>/...`)
    ...
    private func drainSessionFileChangesIntoDirty() async {
        ...
        let baseURL = await ChatStore.shared.iBaseURL
