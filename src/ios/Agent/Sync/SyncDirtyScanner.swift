...
final class SyncDirtyScanner {
    ...
    static func maxModDate(inSkill skillId: String) -> Date {
        let dir = AIChatViewModel.iSkillsPersistentDir.appendingPathComponent(skillId)
    ...
    private func scanConfigFiles() async {
        ...
        let files: [(path: String, recordType: String, recordId: String)] = [
            ("IChat/provider-config.json", "ProviderConfig", "provider-config"),
            ("IChat/env-vars.json", "EnvVar", "env-vars"),
        ]
        for file in files {
        ...
        // IConfig persistent dir (bind-mounted at /var/i/mcp-servers),
        // and is also written by the in-guest i-mcp-cli — so an mtime scan
