...
enum SoulStore {
    ...
    static var fileURL: URL {
        AIChatViewModel.iMemoryPersistentDir.appendingPathComponent("SOUL.md")
    }

    // MARK: - Body length rules (unified token count)
    //
    // The personality body has a single hard cap of 2000 tokens, applied
    // at every write surface (Settings UI Save button, i-config writer,
...
extension Notification.Name {
    ...
    static let soulMdChanged = Notification.Name("ISoulMdChanged")
}
...
enum SystemPromptBuilder {
    ...
    /// expectations ("I, capable AI assistant, iSH Linux shell")
...
/// Renders the current SOUL.md `name` (falling back to "I") and
/// auto-refreshes whenever SoulStore posts `.soulMdChanged`. Use this in
/// any place that previously hard-coded "I" as a label.
@MainActor
