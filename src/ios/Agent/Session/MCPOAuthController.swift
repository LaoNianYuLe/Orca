...
//
//  Ownership split with the in-guest i-mcp-cli:
...
//       authorizes there.
//    2. The guest bridge file (<IConfig>/mcp-servers/oauth/<name>.json,
...
struct MCPOAuthConfig: Codable, Hashable {
    ...
    var scopes: String?
    /// Custom redirect URI. Default i-mcp://oauth/callback; Google installed
...
final class MCPOAuthController: NSObject, ObservableObject {
    ...
    nonisolated private static let keychainService = "com.i.app.mcp-oauth"
    ...
    /// `i-mcp-cli add --oauth-client-secret` can't reach the Keychain from
    ...
    static func importPendingSecretIfAny(server: String) -> Bool {
        let url = AIChatViewModel.iMcpServersPersistentDir
    ...
    /// (bind-mounted at /var/i/mcp-servers/oauth/<name>.json).
    nonisolated static func bridgeFileURL(server: String) -> URL {
        AIChatViewModel.iMcpServersPersistentDir
            .appendingPathComponent("oauth", isDirectory: true)
