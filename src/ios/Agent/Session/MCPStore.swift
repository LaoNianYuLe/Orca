...
//
//  The server list lives in IConfig/mcp-servers/servers.json (mcpServers
//  object form, Claude-Desktop compatible), bind-mounted into the guest at
//  /var/i/mcp-servers — the SAME file the in-guest `i-mcp-cli`
...
struct MCPServerConfig: Codable, Identifiable, Hashable {
    ...
    /// first MCP `initialize` (I config, not MCP protocol). Enforcement
    /// lives entirely in the in-guest `i-mcp-cli` daemon; the native layer
...
final class MCPStore: ObservableObject {
    ...
    /// Host path of the guest's /var/i/mcp-servers/servers.json. Lives in
    /// the IConfig persistent dir (same mechanism as skills/shared/memory:
    ...
    /// ensureISymlinks). NOT iAppGroupRoot: servers.json carries
    ...
    nonisolated static var syncFileURL: URL {
        AIChatViewModel.iMcpServersPersistentDir.appendingPathComponent("servers.json")
    }
    ...
    nonisolated private static var legacyServersFileURL: URL {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("alpine-rootfs/data/var/i/mcp-servers", isDirectory: true)
            .appendingPathComponent("servers.json")
    }

    /// One-time migration of servers.json from the rootfs data tree into the
    /// IConfig persistent dir. Runs before load() on every launch; no-op
    ...
    nonisolated private static func migrateLegacyServersFile() {
        ...
        do {
            ...
            // a symlink by ensureISymlinks/performMount.
    ...
    private var dbPath: String {
        ...
        return library.appendingPathComponent("IChat/skills.db").path
    }
    ...
    // MARK: - servers.json (co-read/write with i-mcp-cli)
    ...
    private struct ServerEntry: Codable {
        ...
        var args: [String]?
        var env: [String: String]?
        var startupTimeoutSeconds: Int?   // I STDIO startup timeout; round-tripped verbatim
    }
    ...
    func systemPromptSnippet(for sessionId: String) -> String? {
        ...
        var lines = "Available MCP Servers (use i-mcp-cli to discover and call):\n"
        for s in selected {
            var note = s.note ?? ""
        ...
        lines += "\nTo use: run `i-mcp-cli tools <server>` to see available tools,\n"
        lines += "then `i-mcp-cli call <server> <tool> [args]` to invoke."
        // Agent-facing guidance on the runtime env placeholder; English-only,
        // not localized (this is prompt text, never shown in the UI).
        lines += "\nWhen adding or modifying an MCP server config (via i-mcp-cli add / the UI), use $$VARNAME in env/headers/url values as a placeholder resolved at runtime from the system/App environment variables — do not hardcode secrets; reference an existing App environment variable as $$NAME."
        return lines
    }

    // MARK: - Tools refresh (native → in-guest CLI) [T-mcp-tools-refresh]

    /// One tool row from `i-mcp-cli tools --refresh`.
    ...
    /// (`i-mcp-cli refresh <name>`), so tools the remote added after the
    ...
    /// it initializes includes /var/i/mcp-servers.
    func refreshTools(server: String) async throws -> [MCPToolInfo] {
        ...
        let result = try await ISHExecutionCoordinator.shared.execute(
            ...
            command: "i-mcp-cli refresh \(quoted)",
            timeout: 120,
            lineCallback: { _ in },
            pidCallback: { _ in }
        )
    ...
    /// Detect edits that bypassed the CRUD paths (in-guest i-mcp-cli
