...
private let logger = AppLogger(category: "AskIIntent")

/// Siri-facing "ask I" entry point. Unlike `SendPromptIntent` (which runs
...
/// conversation — matching the "Hey Siri, ask I to …" experience.
///
...
struct AskIIntent: AppIntent {
    static var title: LocalizedStringResource = "Ask I"
    static var description = IntentDescription("Opens I, sends your prompt, and shows the conversation. Starts a new session, or continues an existing one when you pick a session.")
    ...
    @Parameter(title: "Prompt", requestValueDialog: "What would you like to ask I?")
    var prompt: String
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        if let session = session {
            ...
            if let placeholder = placeholderSid, let realSid = vm.sessionId {
                BackgroundKeepAliveManager.shared.armEagerlyForShortcut(
                    sessionId: realSid, caller: "AskIIntent.swap")
                SessionActivityTracker.shared.setInactive(placeholder,
                    source: "AskIIntent.eager.placeholderSwap")
            }
        }
        ...
        logger.info("AskI send sid=\(sid.prefix(8)) new=\(session == nil)")
        ...
        return .result(dialog: "On it — opening I.")
    }
}
