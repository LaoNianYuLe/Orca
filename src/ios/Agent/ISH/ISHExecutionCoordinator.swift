...
actor ISHExecutionCoordinator {
    ...
    /// handled by IFsRouter's hook, not by bind_mount, so they don't
    /// need to be re-mounted on session switch.
    private var staticMountsInitialized: Set<String> = []

    /// Maps linux mount paths (e.g. "/var/i/memory") to their host
    ...
    /// IFsRouter to resolve their host paths.
    ...
    func execute(
        ...
        let fsContext = IFsRouter.shared.context(for: sessionId)
    ...
    /// routed by IFsRouter's hook now (no bind-mount swap needed), so
    ...
    func mountForSession(_ sessionId: String) {
        ...
        _ = IFsRouter.shared.context(for: sessionId)
        ensureStaticMountsInitialized(for: sessionId)
        mountedSessionId = sessionId
    }
    ...
    /// Resolve a Linux path under /var/i/ to its current host URL.
    /// Tries the static bind-mount table first (memory/skills/shared +
    /// external folders), then falls back to IFsRouter for the per-
    ...
    func hostURL(for linuxPath: String, sessionId: String? = nil) -> URL? {
        ...
        if let sid = sessionId ?? mountedSessionId,
           let url = IFsRouter.shared.hostURL(forGuest: linuxPath, sid: sid) {
            return url
        }
        return nil
    }
    ...
    private func runCommand(
        ...
        // terminator and fails deteritically with
    ...
    private func performMount(_ sid: String) {
        ...
        ensureParentDirsInMetaDB(for: "\(AIChatViewModel.iLinuxBaseDir)/placeholder")
        ensureFakefsMetadata(for: AIChatViewModel.iLinuxBaseDir, isDirectory: true)
        ...
        // routed dynamically by IFsRouter's path-translate hook based on
        ...
        let subdirs: [(persistDir: URL, linuxDir: String)] = [
            (AIChatViewModel.iMemoryPersistentDir, AIChatViewModel.iMemoryLinuxDir),
            (AIChatViewModel.iSkillsPersistentDir, AIChatViewModel.iSkillsLinuxDir),
            (AIChatViewModel.iSharedPersistentDir, AIChatViewModel.iSharedLinuxDir),
            (AIChatViewModel.iMcpServersPersistentDir, AIChatViewModel.iMcpServersLinuxDir),
        ]
        ...
        for (idx, (persistDir, linuxDir)) in subdirs.enumerated() {
            ...
            if let hostDir = resolveHostPath(linuxDir) {
                ...
                if lstatOk && (lstatBuf.st_mode & S_IFMT) == S_IFLNK {
                    ...
                    // Safety: only remove paths under data/var/i/ to avoid accidentally
                    // deleting other fakefs directories (e.g. data/dev/) due to path bugs.
                    let dataVarI = RootfsManager.shared.dataPath.appendingPathComponent("var/i").path
                    guard hostDir.path.hasPrefix(dataVarI) else {
                        logger.error("MOUNT [\(idx)] SAFETY: refusing removeItem outside var/i: \(hostDir.path)")
                        break
                    }
    ...
    private func mountExternalFoldersLocked() {
        ...
        // Ensure the parent /var/i/mounts dir exists in meta.db so fakefs
        // can list it.
        ensureParentDirsInMetaDB(for: "\(AIChatViewModel.iMountsLinuxDir)/placeholder")
        ensureFakefsMetadata(for: AIChatViewModel.iMountsLinuxDir, isDirectory: true)
