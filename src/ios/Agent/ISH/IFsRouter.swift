...
//  IApp
//
//  Routes guest paths under /var/i/{offloads,attachments,workspace,browser}
...
//  hook looks up the token here and rewrites to ~/Library/.../i/<sid>/<bucket>/...
//

import Foundation

final class IFsRouter: @unchecked Sendable {
    static let shared = IFsRouter()

    /// Guest path prefixes that route per-session. Anything under one of these
    /// becomes ~/Library/.../i/<sid>/<bucket>/<tail>. Paths under
    /// /var/i/{memory,skills,shared} stay global and are NOT listed here —
    /// they fall through to the legacy g_bind_mounts[] table.
    private let perSessionBuckets: [(linuxPrefix: String, hostSubdir: String)] = [
        (AIChatViewModel.iOffloadsLinuxDir,    "offloads"),
        (AIChatViewModel.iAttachmentsLinuxDir, "attachments"),
        (AIChatViewModel.iWorkspaceLinuxDir,   "workspace"),
        (AIChatViewModel.iBrowserLinuxDir,     "browser"),
    ]
    ...
    /// Host base URL (~/Library/IChat/i). Captured once at install time.
    private let iBaseURL: URL

    private init() {
        self.iBaseURL = AIChatViewModel.iPersistentBase
    }
    ...
    /// Reverse hook: given a host APFS path under <iBaseURL>/<sid>/<bucket>,
    /// return the canonical guest path /var/i/<bucket>/<tail>.
