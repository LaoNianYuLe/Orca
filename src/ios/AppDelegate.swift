...
final class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(
        ...
        // the wrong (new) session. The .onAppear registration in IApp
...
final class SceneDelegate: NSObject, UIWindowSceneDelegate {
    ...
    func scene(
        ...
        // (or "Open in I") a file, the file:// URL arrives here in
    ...
    // app is ALREADY running, AirDrop / "Open in I" delivers the file URL
    ...
    // cold-launch path above and as IApp's `.onOpenURL`.
    ...
    /// IApp's `.onOpenURL`: a local file goes to `ExternalFileImporter`
    /// (which detects Provider-export JSON and prompts import-vs-attach), and
    /// anything else falls through to `DeepLinkRouter`. Uses
    /// `ShareCoordinator.shared` — the exact instance IApp injects into the
