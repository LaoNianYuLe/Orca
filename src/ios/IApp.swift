...
struct IApp: App {
    ...
    init() {
        ...
        // first render instead of the `.default` stub ("I"). Without this the
        ...
        AgentLiveActivityManager.shared.cleanupStaleActivities(source: "IApp.init")
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                ...
                .id(appLanguage)
                // Confirmation gate for every i-config write. Mounted
                ...
                // for `i://open?session=…&path=…`). Mounted at the
                ...
                .onAppear {
                    ...
                    if #available(iOS 17.0, *) {
                        IShortcutsProvider.updateAppShortcutParameters()
                    }
                    ...
                    // Create /var/i/mounts/<name> symlinks in the fakefs now
    ...
    private func handleScenePhaseChange(_ phase: ScenePhase) {
        ...
        case .background:
            ...
            // outside i.db). Without a checkpoint here a jetsam can drop
    ...
    private static let fileProviderDomain = NSFileProviderDomain(
        identifier: NSFileProviderDomainIdentifier("com.i.app.files"),
        displayName: "I"
    )
    ...
    private static func registerFileProviderDomain() {
        let root = AIChatViewModel.iAppGroupRoot
        ...
        NSFileProviderManager.getDomainsWithCompletionHandler { domains, error in
            ...
            if needsForceReset && alreadyRegistered {
                ...
                let backupRoot = AIChatViewModel.iConfigRoot.appendingPathComponent("_reset_backup_v3", isDirectory: true)
                ...
                NSFileProviderManager.remove(fileProviderDomain, mode: .removeAll) { preservedLocation, removeErr in
                    ...
                    let providerRoot = AIChatViewModel.iAppGroupRoot
                    let fm = FileManager.default
            ...
            let stale = domains.filter { $0.identifier.rawValue.contains("com.i") }
    ...
    /// 1. Library/IChat/shared/ → App Group IFileProvider/shared/
    /// 2. Library/IChat/memory/ → App Group IFileProvider/memory/
    /// 3. Library/IChat/skills/ → App Group IFileProvider/skills/
    /// 4. Old App Group IShared/ → App Group IFileProvider/shared/
    private static func migrateSharedDirToAppGroup() {
        let fm = FileManager.default
        let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.i.app")!

        let migrations: [(source: URL, dest: URL, label: String)] = [
            // Legacy Library/IChat/shared → new shared
            (library.appendingPathComponent("IChat/shared", isDirectory: true),
             AIChatViewModel.iSharedPersistentDir, "shared"),
            // Legacy Library/IChat/memory → new memory
            (library.appendingPathComponent("IChat/memory", isDirectory: true),
             AIChatViewModel.iMemoryPersistentDir, "memory"),
            // Legacy Library/IChat/skills → new skills
            (library.appendingPathComponent("IChat/skills", isDirectory: true),
             AIChatViewModel.iSkillsPersistentDir, "skills"),
            // Old App Group IShared → new shared
            (container.appendingPathComponent("IShared", isDirectory: true),
             AIChatViewModel.iSharedPersistentDir, "IShared→shared"),
        ]
    ...
    /// `i://open?session=…&path=…` deep link (i.app launcher
