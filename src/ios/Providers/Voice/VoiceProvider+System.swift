...
final class SystemVoiceProvider: NSObject, VoiceInputCapable, VoiceOutputCapable {
    ...
    /// `createdAt` is a fixed epoch so the value is stable/deteritic.
    ...
    static func sampleText(forLanguage langCode: String?) -> String {
        ...
        if lc.hasPrefix("zh") { return "你好，这是 I 的语音测试。" }
        if lc.hasPrefix("ja") { return "こんにちは、これは I の音声テストです。" }
        if lc.hasPrefix("ko") { return "안녕하세요, I 음성 테스트입니다." }
        return "Hi! This is I testing this voice."
    }
