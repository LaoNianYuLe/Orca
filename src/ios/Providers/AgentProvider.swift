...
enum AgentContentPart: @unchecked Sendable {
    ...
    /// persisted to (e.g. `/var/i/browser/<sid>/screenshot_*.jpg`,
    /// `/var/i/attachments/uploads/*`). Carried so the request-level
...
enum AgentStopReason: Sendable {
    ...
    /// pointless transient-retry path (a refusal is deteritic, not transient).
