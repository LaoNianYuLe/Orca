...
cat > "$OUT_DIR/examples/i_rpc_android.py" <<'PYCLIENT'
#!/usr/bin/env python3
"""I ANDROID debug server client (stdlib only).

Usage:
    python3 i_rpc_android.py [--host HOST:PORT] [--token TOKEN] <method> [params-json]

Examples:
    adb forward tcp:5321 tcp:5321
    python3 i_rpc_android.py debug.appInfo '{}'
    python3 i_rpc_android.py --host 10.0.0.5:5321 --token "$TOK" debug.logs.list '{}'
...
curl localhost:5321/skill/examples/python > i_rpc_android.py
```
CURLDOC
